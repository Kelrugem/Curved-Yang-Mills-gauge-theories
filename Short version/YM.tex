...
\mathrm{T}_pr_\sigma(X)
	+ \mleft.{\overline{\mleft( \mu_{\mathscr{G}}\mright)_g \mleft(Y - \mathrm{D}_{e_x}R_\sigma \bigl( \mathrm{D}_x \mathds{1} (\omega) \bigr)\mright)}}\mright|_{p \cdot g}
\end{align*}
for all $(p, g) \in N \fibtimes\Phi{\pi_\scG} \scG$ and $(X, Y) \in \rmT N \fibtimes{\rmT\Phi}{\rmT\pi_\scG} \rmT \scG$, where $x \coloneqq \Phi(p) = \pi_\scG(g)$, $\sigma$ is any (local) section of $\mathscr{G}$ with $\sigma_{x} = g$, $\mathds{1}$ is the identity section of $\mathscr{G}$, and $\omega$ is an element of $\mathrm{T}_xM$ given by
\begin{align*}
\omega
&\coloneqq
\mathrm{T}_p \Phi(X)
...
\mathrm{T}_pr_\sigma(X)
	...
	- \mathrm{T}_{e_x}(\Psi_\tau \circ R_\sigma)\bigl( \mathrm{T}_x \mathds{1}(\omega) \bigr)
\end{align*}
and
\begin{align*}
\mathrm{T}_{(p, g)}\Psi(X, Y)
&=
\mathrm{T}_pr_g\Bigl( X - \mathrm{T}_{\mathds{1}_x}\Psi_\tau\bigl( \mathrm{T}_x\mathds{1}(\omega) \bigr) \Bigr)
	+ \mathrm{T}_g\Psi_p \Bigl( Y - \mathrm{D}_{\mathds{1}_x}R_\sigma \bigl( \mathrm{T}_x \mathds{1} (\omega) \bigr) \Bigr)
\nonumber\\
&\hspace{1cm}
	+ \mathrm{T}_{\mathds{1}_x} \mleft( \Phi_\tau \circ R_\sigma \mright)\bigl( \mathrm{T}_x\mathds{1}(\omega) \bigr),
\end{align*}
