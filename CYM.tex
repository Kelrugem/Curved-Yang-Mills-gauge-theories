...
For another important example recall that there is the notion of associated fibre bundles; following \cite[\S1, Construction 1.3.8, page 20]{mackenzieGeneralTheory} and \cite[\S 4.7, page 237ff.; see also Rem.\ 4.7.8, page 242f.]{Hamilton}: Let $P \stackrel{\pi_P}{\to} M$ be a principal bundle with structural Lie group $G$ over a smooth manifold $M$, $N$ another smooth manifold, and $\Psi$ a smooth left $G$-action denoted by
\bas
G \times N &\to N,\\
...
A very important example are of course associated vector bundles, related to $N$ being a vector space, and $\Psi$ a Lie group representation. Similarly one has associated Lie group bundles: 
...
If one has a section of $f$, \textit{i.e.}\ a smooth map $\tau: M \to N$, $x \mapsto \tau_x$, with $f \circ \tau = \mathbb{1}_M$, then we can define the \textbf{orbit map} through $\tau$ as a map $\Phi_\tau$ given by
\bas
\mathcal{G} &\to N,\\
