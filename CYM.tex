...
\bigl(p, \Phi_p(g) \bigr)
\eas
for all $g \in G_p$, which is well-defined since $\Phi_p(g) \in F_{\phi(p)}$. The map $\widetilde{\Phi} \mapsto \Phi \coloneqq \mathrm{pr}_2 \circ \widetilde{\Phi}$ is then a bijective map between base-preserving morphisms $G \to \phi^*F$ and morphisms $G \to F$ over $\phi$, where $\mathrm{pr}_2$ is the projection onto the second component. One can extend this similarly for more specific types of morphisms like vector bundle-morphisms.

Observe that $\widetilde{\Phi}$ is an isomorphism (diffeomorphism) if and only if $\Phi$ is a fibre-wise isomorphism (diffeomorphism).

We will not mention this 1:1 correspondence explicitly, it should be clear by context. Hence, we will also denote $\widetilde{\Phi}$ by $\Phi$. In fact, we usually calculate with $\widetilde{\Phi}$, while $\Phi$ and its diagram may only arise to give an illustration about the geometry.
	%
...
g &\mapsto \mleft.\bigl(\mathrm{D}\mu \circ (0,\nu)\bigr)\mright|_g = \mathrm{D}_{(g, e_x)}\mu\mleft(0_g, \nu_{\pi(g)}\mright) = X_\nu|_g
\eas
...
As one may expect, elements of $L(\mathcal{G})$ are as determined by their value at $e_M$; this can be shown with the Maurer-Cartan form on LGBs, which we will call vertical Maurer-Cartan form. It will be clear later why we choose to add this adjective; however, as a first argument recal Rem.\ \ref{rem:AbstractNotationTwoForLeftInvarVfs}, especially the last paragraph.

\begin{corollaries}{Well-definedness of the vertical Maurer-Cartan form}{VertMCVormIsWellDefined}
Let $\mathcal{G} \stackrel{\pi}{\to} M$ be an LGB over a smooth manifold $M$. Then the following map
\bas
(\mu_\mathcal{G})_g(v)
&\coloneqq
...
is an element of $\Gamma(\mathrm{V}^*\mathcal{G} \otimes \pi^*\mathcal{g})$, where $\mathrm{V}^*\mathcal{G}$ is the dual bundle of $\mathrm{V}\mathcal{G}$.
\end{corollaries}
...
\mu_{\mathcal{G}}(v),
\eas
where $\gamma: I \to \mathcal{G}_{x}$ ($I$ an open interval containing 0, and $x \coloneqq \pi(g)$) is a curve with $\gamma(0)= g$ and $\mathrm{d}/\mathrm{d}t|_{t=0} \gamma = v$. Since $\mu$ is smooth, also $\mathrm{D}\mu \in \Omega^1(\mathcal{G}*\mathcal{G}; \mu^*\mathcal{G})$ is smooth. Denote with $\iota_0$ the contraction 
\end{proof}
...
Let $\mathcal{G} \stackrel{\pi}{\to} M$ be an LGB over a smooth manifold $M$. The \textbf{vertical Maurer-Cartan form $\mu_\mathcal{G}$} is then defined to be an element of $\Gamma\mleft( \mathrm{V}^*\mathcal{G} \otimes \pi^*\mathcal{g} \mright)$ given by
\bas
(\mu_\mathcal{G})_g(v)
&\coloneqq
\mleft( \mathrm{D}_g L_{g^{-1}} \mright)(v)
...
\begin{corollaries}{Tangent space of $\mathcal{G}$, \newline \cite[\S 3.5, a reformulation of Prop.\ 3.5.3 and Cor.\ 3.5.4, page 121]{mackenzieGeneralTheory}}{TLGBAsLGB}
...
\begin{remark}
\leavevmode\newline
Observe that by Cor.\ \ref{cor:PullBackLABIsLAB} we know that $\mathrm{V}\mathcal{G}$ admits a unique LAB structure such that $\mathrm{V}\mathcal{G} \cong \pi^*\mathcal{g}$ is an isomorphism of LABs. This statement is also not in contradiction with $\mathcal{g} = e^*\mathrm{V}\mathcal{G}$, because
\bas
...
\leavevmode\newline
This can be quickly shown by observing that we have the following commuting diagram
\begin{center}
	\begin{tikzcd}
...
We are going to define LGB representations and corresponding LAB representations.
