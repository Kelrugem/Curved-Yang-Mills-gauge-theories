...
As developed in \cite{My1stpaper, MyThesis}: There is an equivalence relation of curved Yang-Mills gauge theories, preserving the dynamics and kinematics of this theory. Let us recapitulate and integrate these:

\begin{definitions}{Field redefinitions on the LGB}{FieldRedefsAsDefIntegrated}
Let $\pi \colon \mathcal{G} \to M$ be an LGB over a smooth manifold $M$ with LAB $\mathcal{g}$; furthermore let $\mu_{\mathcal{G}}^{\mathrm{tot}}$ be a Yang-Mills connection on $\mathcal{G}$ (w.r.t.\ a $\zeta \in \Omega^2(M; \cag)$). We then define \textbf{field redefinitions w.r.t.\ $\lambda \in \Omega^1(M; \mathcal{g})$} of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ by
\bes
\mleft.\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}\mright|_g
\coloneqq
\mleft.\mu_{\mathcal{G}}^{\mathrm{tot}}\mright|_g
	+ \sAd_{g^{-1}} \circ \mleft.\pi^!\lambda\mright|_g
	- \mleft.\pi^!\lambda\mright|_g
\ees
for all $g \in \caG$
and of $\zeta$ by
\bes
...
\begin{proof}[Proof of Prop.\ \ref{prop:YMStaysYM}]
\leavevmode\newline
...
\mu_{\mathcal{G}}^{\mathrm{tot}}
	+ \delta \lambda ~.
\ees
Thus, $\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}$ stays closed, and thus it is multiplicative. That it satisfies the generalised Maurer-Cartan equation w.r.t.\ $\zeta^\lambda$ is a straight-forward but a bit lengthy calculation, which is why the keep this as an exercise for the reader; for the infinitesimal, i.e.\ in the connected case by Thm.\ \ref{thm:GenMCEq}, see \cite{My1stpaper, MyThesis}.
\end{proof}

In fact, a simple field redefinition of an Ehresmann connection preserves the compatibilities with the new multiplicative connection; for the following observe that the field redefinition of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ induces a field redefinition of $\nabla^{\mathrm{YM}}$ given by
\bes
...
\begin{proof}
\leavevmode\newline
That $F^\lambda = F$ follows by similar calculations as in \cite{My1stpaper, MyThesis}. Therefore let us only prove that $A^\lambda$ is still a connection 1-form even though we add a form which does not transform by the Adjoin representation in general; the trick is to observe that $A^\lambda$ is a connection 1-form w.r.t.\ the field redefined connection on $\mathcal{G}$. $A^\lambda$ is clearly still the identity on the vertical structure, and henceforth let us check its behaviour w.r.t.\ the modified pushforward induced by $\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}$, denoted by $\mathcal{r}_\sigma^\lambda$ for $\sigma \in \Gamma(\mathcal{G})$: We have to study
\bas
\mleft(\mleft(\mathcal{r}_\sigma^\lambda\mright)^!A^\lambda\mright)_p(X)
\eas
for all $p \in \mathcal{P}_x$ and $X \in \mathrm{T}_p\mathcal{P}$. First of all, similar to the beginning of the proof of Prop.\ \ref{prop:NewFieldStrengthWithCoolProps} we have
\bes
...
Thus, the field redefinition of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is given by
\bas
\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}
&\coloneqq
\mu_{\mathcal{G}}^{\mathrm{tot}}
...
\eas
for all $g \in \mathcal{G}_x$, and $\widetilde{\pi_{\mathcal{G}}^! \lambda}^l$ is just the typical definition for fundamental vector fields, with an emphasis on that it is via left-multiplication. We will later show that $A^\lambda$ is a connection 1-form on $\mathcal{P}$ w.r.t.\ $\widetilde{\mu_{\mathcal{G}}^{\mathrm{tot}}}^{\lambda}$.

Prop.\ \ref{prop:YMStaysYM} has now several interpretations and implications, where we denote with $\mleft[\mu_{\mathcal{G}}^{\mathrm{tot}}\mright]_{\delta}$ the equivalence class of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ inherited by $\delta$:
\begin{enumerate}
...
\widetilde{\nabla^{\mathrm{YM}}}^\lambda
&\coloneqq
\nabla^{\mathrm{YM}} - \mathrm{ad} \circ \lambda,
\\
...
\mathrm{D}_{e_x}R_g \circ \lambda_{x} \circ \mathrm{D}_x \pi_{\mathcal{G}}
\eas
for all $g \in \mathcal{G}_x$, and $\widetilde{\pi_{\mathcal{G}}^! \lambda}^l$ is just the typical definition for fundamental vector fields, with an emphasis on that it is via left-multiplication. It is now a straight-forward exercise to show that $\widetilde{A}^\lambda$ is a connection 1-form on $\mathcal{P}$ w.r.t.\ $\widetilde{\mu_{\mathcal{G}}^{\mathrm{tot}}}^{\lambda}$.

Thm.\ \ref{thm:FieldRedefStuff} has now several interpretations and implications, where we denote with $\mleft[\mu_{\mathcal{G}}^{\mathrm{tot}}\mright]_{\delta}$ the equivalence class of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ inherited by $\delta$:
\begin{enumerate}
	\item The field redefinition of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is about picking any other representative for $\mleft[\mu_{\mathcal{G}}^{\mathrm{tot}}\mright]_{\delta}$.
	\item Every other representative of $\mleft[\mu_{\mathcal{G}}^{\mathrm{tot}}\mright]_{\delta}$ is not just multiplicative, but also a Yang-Mills connection, that is,
	\bas
	\delta \widetilde{\mu_{\mathcal{G}}^{\mathrm{tot}}}^{\lambda} &= 0,\\
	\widetilde{F_{\mathcal{G}}}^{\lambda} &= \delta\widetilde{\zeta}^{\lambda},
	\eas
