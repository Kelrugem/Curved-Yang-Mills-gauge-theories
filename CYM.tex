...
\eas
for all $g \in \mathcal{G}_x$ ($x \in M$). By construction $\mathrm{Ad}_g \in \mathrm{Aut}(\mathcal{g}_x) = \mleft.\mathrm{Aut}(\mathcal{g})\mright|_x$ so that the adjoint representation is well-defined. Due to the fact that $c_{gq} = c_{g} \circ c_q$ for all $g, q\in \mathcal{G}_x$ and $c_g(e_x) = e_x$ we have
\bas
\mathrm{Ad}_{gq}
&=
...
\eas
Smoothness can be shown similarly to the proof in \cite[\S 2.1, Thm.\ 2.1.45, page 101f.]{Hamilton}
\end{examples}
