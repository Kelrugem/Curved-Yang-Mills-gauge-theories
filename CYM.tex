...
&=
\mathrm{pr}_2\biggl(\mathrm{D}_{\mathrm{D}_x e(\lambda X + \kappa Y)}\mu_{\mathcal{G}}^{\mathrm{tot}} \Bigl( S_{\mathcal{G}} \bigl( \mathrm{D}_x\nu (\lambda X + \kappa Y) \bigr) \Bigr) \biggr)
\eas
for all $\lambda, \kappa \in \mathbb{R}$, using Remark \ref{rem:TotalDerivativesAreLinearWithRTOtherLinearStructure} and \ref{rem:BothLinearStructuresTheSameOnTheVerticalBundle}.
\end{proof}
