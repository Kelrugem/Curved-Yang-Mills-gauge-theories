...
$\bullet$ Let us first assume that $\mathrm{H}\mathcal{G}$ is an Ehresmann connection, therefore we now initially show that the associated total Maurer-Cartan form is multiplicative. By Cor.\ \ref{cor:TotMCFormIsConnectionForm} we know that $\mu_{\mathcal{G}}^{\mathrm{tot}}$ satisfies Def.\ \ref{def:GaugeBosonsOnLGBPrincies}; also recall the second part of Rem.\ \ref{PointwiseNotationOfConnectioNOneForms}. By additionally using Thm.\ \ref{thm:DiffOfLGBAction} (especially the associated Rem.\ \ref{rem:DarbouxInActionInfinit}) we show
\bas
...
\sAd_{q^{-1}}\mleft(\mleft(\mu_{\mathcal{G}}^{\mathrm{tot}}\mright)_{g}(X)\mright)
		+ \mleft( gq, \mleft( \widehat{\mu_{\mathcal{G}}^{\mathrm{tot}}}\mright)_{q} (Y) \mright)
\eas 
for all $(g, q) \in \mathcal{G}*\mathcal{G}$ and $(X, Y) \in \mathrm{T}_{(g, q)}(\mathcal{G}*\mathcal{G})$, where $\sigma$ is a (local) section of $\mathcal{G}$ such that $\sigma_{x} = q$. Thus, $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is multiplicative.

$\bullet$ Now we assume that $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is multiplicative. In order to show that $\mathrm{H}\mathcal{G}$ we just need to show that $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is a connection 1-form on $\mathcal{G}$ due to the fact that $\mathrm{H}\mathcal{G}$ is the kernel of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ by definition, using Thm.\ \ref{thm:OurConnectionHasAUniqueoneForm}. By definition, $\mu_{\mathcal{G}}^{\mathrm{tot}}$ already satisfies
\bas
