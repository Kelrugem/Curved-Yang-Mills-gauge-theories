...
\mleft[ \mu_x, \nu_x \mright]_{\cag}
\coloneqq
\mleft.\mleft[ X_\mu, X_\nu \mright]_{\rmT\caG}\mright|_{e_x}
\end{equation*}
...
\begin{corollaries}{The exponential map as flow of $L(\mathcal{G})$}{ExpAsFlow}
...
Recall the notation introduced in Def.\ \ref{def:LeftRightTranslationConjugation}:
...
\begin{proof}[Proof of \ref{prop:DiffOfLGBAction}]
\leavevmode\newline
...
\begin{remark}
\leavevmode\newline
Eq.\ \eqref{NewDIffACtionwithMCForm} is very similar to the "classical" formula used in gauge theory, see \textit{e.g.}\ \cite[\S 3.5, Prop.\ 3.5.4, page 149]{Hamilton}: In the case of a Lie group action on $N$ we have
\bas
\mathrm{D}_{(p, g)}\Phi(X, Y)
&=
\mathrm{D}_pr_g(X)
...
&=
\mathrm{D}_pr_\sigma(X)
	- \mleft.{\overline{ \mleft.\mleft(\pi^!\Delta \sigma\mright)\mright|_p (X)}}\mright|_{p \cdot \widetilde{\sigma}_p}
	+ \mleft.{\overline{\mleft( \mutot\mright)_{\widetilde{\sigma}_p} \bigl(\mathrm{D}_p\widetilde{\sigma}(X) \bigr)}}\mright|_{p \cdot \widetilde{\sigma}_p}
\nonumber
\\
&=
...
\\
&\hspace{1cm}
	- \mleft(\mutot\mright)_g\mleft( [\widetilde{\nu}, \widetilde{\mu}] \mright)
	- \mleft(\mutot\mright)_g\mleft( [\widetilde{\nu}, X] \mright)
\\
&=
...
and
\bas
\mleft(\mathrm{d}^{\pi^*\nabla^\caG} A\mright)\mleft( \widetilde{\nu}, \widetilde{\mu} \mright)
&=
{\underbrace{\mleft(\pi^*\nabla^\caG\mright)_{\widetilde{\nu}} \bigl(A\mleft( \widetilde{\mu} \mright) \bigr)}_{\mathclap{ = \pi^*\mleft( \nabla^\caG_{\mathrm{D}\pi\mleft( \widetilde{\nu} \mright)} \mu \mright) = 0 }}}
	- \mleft(\pi^*\nabla^\caG\mright)_{\widetilde{\mu}} \bigl(A\mleft( \widetilde{\nu} \mright) \bigr)
...
\bas
\mleft(\mathrm{d}^{\pi^*\nabla^\caG} A\mright)\mleft( X, \widetilde{\nu} \mright)
&=
...
&\coloneqq
\pi^{\mathrm{vert}}
	+ \widetilde{\pi_{\mathcal{G}}^! \lambda}^l
	+ \widetilde{\pi_{\mathcal{G}}^! \lambda}^r,
\eas
...
\bas
\mleft.\widetilde{\pi_{\mathcal{G}^!} \lambda}^r\mright|_g
&=
\mathrm{D}_{e_x}R_g \circ \lambda_{x} \circ \mathrm{D}_x \pi_{\mathcal{G}}
\eas
for all $g \in \mathcal{G}_x$, and $\widetilde{\pi_{\mathcal{G}}^! \lambda}^l$ is just the typical definition for fundamental vector fields, with an emphasis on that it is via left-multiplication. 
\end{remark}

\begin{propositions}{Field redefined multiplicative Yang-Mills connection a multiplicative Yang-Mills connection}{YMStaysYM}
Given the context of Def.\ \ref{def:FieldRedefsAsDefIntegrated}, the field redefinition of $\mutot$ is a multiplicative Yang-Mills connection w.r.t.\ $\widetilde{\zeta}^\lambda$.
\end{propositions}
