...
\leavevmode\newline
The notation with the pullback $\sigma^*$ is only needed if one wants to view $\Delta \sigma$ as a $C^\infty(M)$-linear map $\mathfrak{X}(M) \to \Gamma(\mathcal{g})$; in this case it is a composition of base-preserving vector bundle morphisms $\mathrm{D}\sigma: \mathrm{T}M \to \sigma^*\mathrm{T}\mathcal{G}$ and $\sigma^*\mutot: \sigma^*\mathrm{T}\mathcal{G} \to \sigma^*\pi_{\mathcal{G}}^*\mathcal{g} \cong \mathcal{g}$ which can be extended to sections, where $\pi$ is the projection of $\mathcal{G}$.

Of course one can view $\Delta \sigma$ as a map $\mathrm{T}M \to \mathcal{g}$ which one can also write as the composition of $\mathrm{D}\sigma: \mathrm{T}M \to \mathrm{T}\mathcal{G}$ and $\mutot: \mathrm{T}\mathcal{G} \to \mathcal{g}$, that is,
\begin{align*}
\Delta \sigma
&=
\mutot \circ \mathrm{D}\sigma.
\end{align*}
If one wants to emphasize base points of the involved pullback bundles, that is, one views $\mutot$ as a map $\mathrm{T}\mathcal{G} \to \pi_{\mathcal{G}}^*\mathcal{g}$, then one can write point-wise
\begin{align*}
...
Recall Thm.\ \ref{prop:DiffOfLGBAction}; keeping the same notation as in this theorem but denoting the projection of $\mathcal{P}$ by $\pi$, we checked several times that $Y - \mathrm{D}_{x}\sigma (\omega)$ is vertical, and thus we can now write
\begin{align*}
\mathrm{D}_{(p, g)}\Phi(X, Y)
&=
\mathrm{D}_pr_\sigma(X)
