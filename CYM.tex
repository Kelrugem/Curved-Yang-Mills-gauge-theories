...
$\bullet$ To show the smoothness of the proposed action we simply use the atlas constructed in the proof for Thm.\ \ref{thm:AssociatedGroupBundlesHaveGroupStructure}; the idea of the atlas there does not just work for $c_G(P)$ but it also serves for constructing a fibre bundle atlas for $\mathcal{F}$ following the same relevant part of the proof. That is, we will use a principal bundle atlas for $P$, hence for some $U \subset M$ open and a local trivialization $\varphi_U: P_U \to U \times G$ of $P$ we write
\bas
\varphi_U(p)
&=
\bigl( \pi_P(p), \beta_U(p) \bigr)
\eas
for all $p \in P$, where $\beta_U: P_U \to G$ is an equivariant map. Then we have a map $\phi_{c_G(P)}$ by 
\bas
\mleftc_G(P)\mright|_U
&\to
U \times G,\\
[p, h]
&\mapsto
