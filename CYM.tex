...
This leads to a generalized gauge theory with many similar, but generalized, statements, including Bianchi identity, gauge transformations and Darboux derivatives. Last but not least, we will show that this gauge theory is an integral of the infinitesimal gauge theory developed by Alexei Kotov and Thomas Strobl, called curved Yang-Mills gauge theory.
}
 \end{abstract}
\end{center}
...
\eas
for all $(p, g) \in \mathcal{P} * \mathcal{G}$ and $(X, Y) \in \mathrm{T}_{(p, g)}(\mathcal{P}*\mathcal{G})$, where $x \coloneqq \pi(p)$, $\sigma$ is any (local) section of $\mathcal{G}$ with $\sigma_{x} = g$, $e$ is the identity section of $\mathcal{G}$, and $\omega$ is an element of $\mathrm{T}_xM$ given by
\bas
\omega
&\coloneqq
\mathrm{D}_p \pi(X)
