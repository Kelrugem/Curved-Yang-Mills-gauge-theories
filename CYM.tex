...
\begin{proof}
\leavevmode\newline
This result can be proven similarly as for Lie group based principal bundles. First of all, due to the fact that $H$ is base-preserving we know that $H(p)$ is in the same fibre as $p$, and due to that the $\mathcal{G}$-action on $\mathcal{P}$ is simply transitive there is a unique element of $\mathcal{G}$, denoted by $\sigma^H_p$, such that
\bas
H(p)
&=
p \cdot \sigma_p^H.
\eas
$p \mapsto \sigma_p^H$ is smooth due to Remark \ref{rem:LGBPrincDefDiscussion}: We can write
\bas
...
\bas
\sigma_p^H = \mleft( \Phi_p \mright)^{-1}\bigl( H(p) \bigr).
\eas
...
By definitions, if we fix a point $x \in M$, then $H_x$ is an element of $\sAut(\mathcal{P}_x)$, where $\mathcal{P}_x$ is viewed as principal $\mathcal{G}_x$-bundle. Elements of $C^\infty(\mathcal{P}; \mathcal{G})^{\mathcal{G}}$ restricted to $\mathcal{P}_x$ are precisely the elements of $C^\infty(\mathcal{P}_x; \mathcal{G}_x)^{\mathcal{G}_x}$
\end{proof}
