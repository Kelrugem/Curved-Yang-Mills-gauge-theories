...
Observe that $(g, v) \in \mathcal{G}*V$ means that $p(v) = \pi(g)$, same for $(g, w)$. Hence, given a base point in $x\in M$, the pairs $(g, v)$ in $\mathcal{G}*V|_{p^{-1}(\{x\})}$ are given by elements $g \in \mathcal{G}_x$ and $v \in V_x$. 
