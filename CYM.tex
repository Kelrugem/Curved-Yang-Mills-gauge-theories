...
f^*\mathcal{G},
\ea
such that the action's notation has the typical shape $N * \mathcal{G} \to N$; one may also write $N * \mathcal{G} = N \times_M \mathcal{G}$. For left actions similarly $\mathcal{G} * N \to N$. 
\end{definitions}
...
f(p \cdot g) &= f(p),
\eas
so, the $\mathcal{G}$-action is defined in such a way that $f$ is invariant under it. If $f$ is the projection of $N$ as a bundle over $M$, then this means that an LGB action is fibre-preserving. Moreover, the fibre-wise group structure on $\mathcal{G}$ naturally defines a $\mathcal{G}$-action on $\mathcal{G}$; in this situation $f$ would be $\pi$ itself. This is mainly a technical condition. On one hand, having $M = \{*\}$ already recovers the notion of a Lie group action and condition \eqref{InvarianceOffUnderGAction} is then trivial, and on the other hand the mentioned reference, \cite[\S 1.6, Def.\ 1.6.1, page 34]{mackenzieGeneralTheory}, actually generalizes this condition making use of the structure of groupoids.
\newline
...
\textbf{Add 1-parameter subgroups, which are sections of LGB at each time.}
...
\textbf{Add induced representations, especially adjoint ones}
...
However, if $f$ is \textit{e.g.}\ the projection of a bundle, then we have a $\mathcal{G}$-action on the fibre $f^{-1}(\{x\})$ as manifold, and therefore $\widetilde{v}|_{f^{-1}(\{x\})}$ and $\widetilde{\nu_x}$ give rise to a vector field on $f^{-1}(\{x\})$. In other words, fundamental vector fields are vertical vector fields as expected.
...
\ea
where $\tau$ is any additional (local) section of $f$\footnote{That is, $f \circ \tau = \mathds{1}_M$ (locally).} with $\tau_x = p$.
\end{theorems}
...
\subsection{Toy model}\label{TheBigMotivationBehindEverything}
...
\begin{enumerate}
	\item The mentioned reference, \cite[simplification of the beginning of \S 5.7, page 144f.]{GroupoidBasedPrincipalBundles}, introduces principal bundles slightly different; we will come back later to this in Remark \ref{AlternativePrincBdlDef}.
...
We often refer to principal bundles related to trivial LGBs as \textbf{pre-classical principal bundles}. We will later clarify why "pre-classical" as adjective instead of "classical"; however, at this point both adjectives are permutable.
\end{examples}

\begin{examples}{The "trivial" $\mathcal{G}$-principal bundle}{TrivialPrincAsLGB}
...
Our main example will be the inner bundle of a pre-classical principal bundle, recall Ex.\ \ref{ex:InnerLGBs}. 

To explain the "triviality" of the previous example we need to introduce morphisms of principal bundles.
%
%\subsection{Morphism of principal LGB-bundles}
%
...
As expected, there is a natural isomorphism induced by local sections of $\mathcal{P}$, which are, however, no trivializations in general.
...
Let $\mathcal{G} \stackrel{\pi_{\mathcal{G}}}{\to} M$ be an LGB over a smooth manifold $M$, and $\mathcal{P} \to M$ a principal $\mathcal{G}$-bundle. Let $s: U \to \mathcal{P}$ be a smooth local section of $\mathcal{P}$ over an open subset $U$ of $M$. Then the orbit map $H \coloneqq \Phi_s$ through $s$ given by
\bas
...
\leavevmode\newline
The proof is similar to the "classical" statement as \textit{e.g.}\ given in \cite[\S 4.2, proof of Lemma 4.2.7, page 210ff.]{Hamilton}. $H$ is well-defined because of $\pi_{\mathcal{P}} \circ s = \mathds{1}_{U}$ such that $\mleft(s_{\pi_{\mathcal{G}}(g)}, g\mright) \in \mathcal{P}*\mathcal{G}$. Let us denote with $\Phi: \mathcal{P}* \mathcal{G} \to \mathcal{G}$ the right $\mathcal{G}$-action $(p, g) \mapsto p \cdot g$ on $\mathcal{P}$, then
...
\ea
Thence, $H$ is clearly a smooth map as the composition of smooth maps; the second map $\mleft( \pi_{\mathcal{G}}^*s, \mathds{1}_{\mathcal{G}} \mright): \mathcal{G} \times \mathcal{G} \to \mathcal{G}$ is given by $(g, q) \mapsto \mleft( s_{\pi_{\mathcal{G}(g)}} , g \mright)$ and is clearly smooth if restricted to the diagonal as embedded submanifold. By Def.\ \ref{def:LiegroupACtion} we have
\bas
\mleft( \pi_{\mathcal{P}} \circ H \mright)(g)
&=
\pi_{\mathcal{P}}\mleft( s_{\pi_{\mathcal{G}}(g)} \cdot g \mright)
=
\pi_{\mathcal{G}}(g),
\eas
thus, $H$ is base-preserving, similarly it follows that $H$ is $\mathcal{G}$-equivariant w.r.t.\ $\mathds{1}_{\mathcal{G}}$ by making use of $\pi_{\mathcal{G}}(g q) = \pi_{\mathcal{G}}(g)$ for all $g, q \in \mathcal{G}*\mathcal{G}$. Furthermore, $H$ is bijective since the right $\mathcal{G}$-action on $\mathcal{P}$ is simply transitive, that is, when we restrict $H$ to an $x \in U$, then it is a map
\bas
\mathcal{G}_x &\to \mathcal{P}_x,\\
g &\mapsto s_x \cdot g,
\eas
thence, $H_x = \Phi_{s_x}$, the orbit map through $s_x$ which is a $\mathcal{G}_x$-equivariant diffeomorphism by Remark \ref{rem:LGBPrincDefDiscussion}. So, $H$ is fibre-wise bijective and therefore bijective as a whole since it is base-preserving.

Now we want to use the inverse function theorem to show that its inverse is also smooth. Once we know that the tangent map/total derivative $\mathrm{D}_gH: \mathrm{T}_g \mathcal{G} \to \mathrm{T}_{s_{x} \cdot g}\mathcal{P}$ is an isomorphism of vector spaces for all $g \in \mathcal{G}|_U$, then we know by the inverse function theorem that $H^{-1}$ is smooth. Hence, we will now show that $\mathrm{D}_gH$ is injective, then it has to bijective by dimensional reasons so that we are done. This differential is given by Thm.\ \ref{thm:DiffOfLGBAction}, making use of Eq.\ \eqref{RewritingLocalTrivOfPrinc},
\bas
\mathrm{D}_g H (Y)
&=
...
\mathrm{D}_x \mleft( r_\sigma \circ s \mright)(\omega)
	+ \mleft.\oversortoftilde{\mleft( \mu_{\mathcal{G}} \mright)_g\bigl( Y - \mathrm{D}_x \sigma(\omega) \bigr) }\mright|_{s_x \cdot g}
\eas
for all $Y \in \mathrm{T}_g \mathcal{G}$, where $x \coloneqq \pi_{\mathcal{G}}(g)$, $\sigma \in \Gamma(\mathcal{G}|_U)$\footnote{W.l.o.g.\ we assume that $\sigma$ is defined on $U$, otherwise "make $U$ smaller".} with $\sigma_{x} = g$ and $\omega \coloneqq \mathrm{D}_{g}\pi_{\mathcal{G}}(Y) \in \mathrm{T}_xM$. We want to decompose $H$ now. Observe that $Y - \mathrm{D}_x \sigma(\omega) \in \mathrm{V}_g \mathcal{G}$, \textit{i.e.}\ it is vertical in $\mathcal{G}$ due to the fact that
...
\begin{lemmata}{$\mathcal{P}*\mathcal{G}$ is the pullback of $\mathcal{P}$ along its projection}{ProductSpaceIsPItself}
...
\eas
w.r.t.\ the LGB isomorphism the identity map on $\mathcal{P} * \mathcal{G}$.
\end{lemmata}
...
(p, g) &\mapsto (p, p \cdot g).
\eas
This is clearly smooth and a base-preserving principal $\pi^*\mathcal{G}$-bundle morphism w.r.t.\ the LGB isomorphism $\mathds{1}_{\mathcal{P}*\mathcal{G}} = \mathds{1}_{\pi^*\mathcal{G}}$. It is also bijective because the right $\mathcal{G}$-action on $\mathcal{P}$ is simply transitive. The inverse is given by
\bas
\pi^*\mathcal{P} &\to \mathcal{P} * \mathcal{G},\\
\mleft(p, p^\prime\mright)
&\mapsto
\mleft(
	p, \Phi^{-1}_p\mleft( p^\prime \mright)
\mright),
\eas
where $\Phi_p$ is the orbit map through $p$, a diffeomorphism by Remark \ref{rem:LGBPrincDefDiscussion}.
%This map is equivalent to
%\bas
%\mathcal{P} * \mathcal{G} &\to \pi^*\mleft(\mathcal{P}*\mathcal{G}\mright) \to \pi^*\mathcal{P},\\
%(p, g)&\mapsto (p, p, g) \mapsto (p, p \cdot g).
%\eas
%The first arrow is clearly a diffeomorphism, since it is the restriction of the image of $\mathcal{P} * \mathcal{G} \to \mathcal{P}\times \mleft(\mathcal{P}*\mathcal{G}\mright)$, $(p, g) \mapsto (p, p, g)$, onto the embedded submanifold $\pi^*\mleft(\mathcal{P}*\mathcal{G}\mright)$; the inverse is given by the smooth projection of $(p, p, g) \mapsto (p,g)$. The second arrow is the map
\end{proof}
