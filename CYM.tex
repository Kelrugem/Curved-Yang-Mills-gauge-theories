...
\leavevmode\newline
The idea of the proof is similar to the proof of a statement about when two multiplicative 2-forms are equal, see \textit{e.g.}\ \cite[\S 3, Cor.\ 3.4]{bursztyn2004integration}. That is, we now first discuss when 
\bas
...
- \mleft( g, \mleft[ \nu_x, \mu_x \mright]_{\mathcal{g}_x} \mright)
\eas
using Remark \ref{rem:FundVecsAreLABActions} (bracket of fundamental/left-invariant vector fields is fundamental/left-invariant) and Lemma \ref{lem:BracketVertHor}. Thus,
\bas
\mleft.\mleft(\mathrm{d}^{\pi_{\mathcal{G}}^*\nabla^{\mathcal{G}}} \mu_{\mathcal{G}}^{\mathrm{tot}}
...
For a given $g \in \mathcal{G}_x$ ($x \in M$) take any curve $\gamma: I \to \mathcal{G}_x$ ($I$ an open interval of $\mathbb{R}$ containing 0) so that $\gamma(0) = g$.
\end{proof}
