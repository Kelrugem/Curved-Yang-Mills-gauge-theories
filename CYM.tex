...
Several examples for a gauge theory with a curved Maurer-Cartan form $\mu$ will be provided, including the inner group bundle of the Hopf fibration $\mathds{S}^7 \to \mathds{S}^4$, and a classification for semisimple group bundles will be provided. We conclude that this gauge theory is an integral of an infinitesimal gauge theory developed by Alexei Kotov and Thomas Strobl.
}
 \end{abstract}
\end{center}
...
Last but not least, these field redefinitions are not just any transformation preserving the physics. As pointed out in \cite{MyThesis}, curved Yang-Mills gauge theory is a reformulation of classical gauge theory in such a way that gauge theory is form-invariant w.r.t.\ the field redefinitions, similar to how one reformulates Classical Mechanics so that it is form-invariant w.r.t.\ Galilei or Lorentz transformations. This reformulation introduces extra terms, $\zeta$ and connection 1-forms of $\nabla$, that is, every classical gauge theory is equivalent to a possibly curved theory with a non-zero $\zeta$ contributing to the field strength. From that point of view it is tempting to ask whether there are examples which cannot be flattened by field redefinitions:
...
There is a natural class of multiplicative Yang-Mills connections which cannot be flattened via field redefinitions. In order to understand this, we follow similar arguments as in the construction recipe I of~\cite[see Ex.\ 1.12]{Fischer:2401.05966}: Assume an ordinary principal $\sfH$-bundle $P$, where $\sfH$ is a Lie group. Also assume that we have a manifold $\sfT$ on which $\sfH$ acts (from the left).\footnote{$\sfT$ and its related constructions and assumptions are only needed to provide an extra geometric interpretation as in \ref{tab:ComparingTheGeometryGaugeTheoryWithSingularFoliations}.} Then we have two associated bundles: 
    ...
    \begin{examples}{Cooking recipe}{Hopf}
        Using a non-trivial principal bundle $P$ over a simply connected base manifold, such as the Hopf fibration $\mathbb{S}^7 \to \mathbb{S}^4$, leads to curved multiplicative Yang-Mills connections on $c_\sfH(P)$ in the above construction.
    \end{examples}
		
		What about the full theory now? For this we need to require the existence of an $\mathrm{Ad}$-invariant connection on $c_H(P)$ which is simply given by assuming that $H$ is semisimple what is covered by the previous discussion; recall Rem.\ \ref{rem:JustifyingExample}. Thus, it is only left to find a principal $c_H(P)$-bundle $\scP$; for this we have several canonical choices:
		\begin{itemize}
