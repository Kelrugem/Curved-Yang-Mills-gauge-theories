...
T &\mapsto L_i \circ T \circ L_i^{-1},
\eas
...
Last but not least, by fixing a base point $x \in M$ we clearly have the typical notion of a $\mathcal{G}_x$-representation on $V_x$. Equivalently, we could therefore obviously define LGB representations as a base-preserving LGB morphism $\Psi: \mathcal{G} \to \mathrm{Aut}(V)$ as it is actually done in the definition's reference.
\end{remarks}
...
As expected, we have a corresponding connection 1-form.
