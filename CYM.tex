...
As mentioned in Remark \ref{rem:ItIsAPrincipalAction}, constructions of quotients related to the LGB action may be possible here. Thus, using the last proposition, one should be able to construct associated bundles in this more general setting. We mainly need this proposition for the following example.
...
For the following definition recall Ex.\ \ref{ex:LGBAdjointRep} and the isomorphism for $\mathrm{V}\mathcal{P}$ in Cor.\ \ref{cor:VerticalBundleOfPrincIsNearlyAsUsual}; also recall the adjoint $\mathcal{G}$-representation on $\mathrm{V}\mathcal{P}$, Ex.\ \ref{ex:AdjointACtionOnVP}.
...
\pi^*\nu
\eas
for all (local) sections $\nu \in \Gamma(\mathcal{g})$. This implies that $A$ has not only values in $\mathrm{V}\mathcal{P}$, but acts also as identity on $\mathrm{V}\mathcal{P}$. Thus, $\mathrm{H}\mathcal{P} = \mathrm{Ker}(A_p)$ is a complementary subspace of $\mathrm{V}_p\mathcal{P}$ for all $p \in \mathcal{P}$. In order to show that $\mathrm{H}\mathcal{P}$ is a horizontal distribution of $\mathcal{P}$, we fix a local frame $\mleft( e_a \mright)_a$ of $\mathcal{g}$ over $U$ (an open subset of $M$), so that $\mleft( \pi^*e_a \mright)_a$ is a frame of $\pi^*\mathcal{g}|_{\pi^{-1}(U)}$. Therefore we write
\bas
A
&=
A^a \otimes \pi^*e_a,
...
\delta^a_b ~ \pi^*e_a,
\eas
where $\delta^a_b$ is the Kronecker delta. By Cor.\ \ref{cor:VerticalBundleOfPrincIsNearlyAsUsual} $\mleft( A^a \mright)_a$ is the dual frame of $\mleft( e_a \mright)_a$, thus, the $A^a$ are linear independent to each other. 
\end{proof}
