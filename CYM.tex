...
\eas
So, in total we have a smooth map $\alpha: I^2 = I \times I \to M$ satisfying
...
Now observe that the flow $X$ of $\xi$ has values in $\mathrm{T}M$, that is, for all $t \in I$ we have a curve $\alpha_t: I \to M$ (w.l.o.g.\ defined on the same interval as $X$), $s \mapsto \alpha(s)$, so that 
\bas
...
\omega.
\eas
So, in total we have for all $\xi \in \mathrm{T}\mathrm{T}M$ a smooth map $\alpha: I^2 = I \times I \to M$, $(t, s) \mapsto \alpha_t(s)$, such that
\bas
