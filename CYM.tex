...
Our main example will be the inner bundle of a pre-classical principal bundle, recall Ex.\ \ref{ex:InnerLGBs}. To explain the "triviality" of the previous example we need to introduce morphisms of principal bundles.
...
\begin{tikzpicture}[scale=1.2]
		...
		%Labels
		\draw (3.2,1) node {\textcolor[rgb]{0,0.58,0}{$\mathrm{H}\mathcal{P}_{p \cdot g}$}};
		\draw (3.2,-1) node {\textcolor[rgb]{0,0.58,0}{$\mathrm{H}\mathcal{P}_p$}};
		\draw (5.5,0) node {\textcolor[rgb]{0,0,1}{$\cdot g$}};
	...
	\begin{tikzpicture}[scale=1.2]
		...
		\draw [<-,dotted,thick] (4.63,2) -- (4.71,1.12);
		%Labels
		\draw (3.2,1) node {\textcolor[rgb]{0,0.58,0}{$\mathrm{H}\mathcal{P}_{p \cdot \sigma_x}$}};
		\draw (3.2,-1) node {\textcolor[rgb]{0,0.58,0}{$\mathrm{H}\mathcal{P}_p$}};
		\draw (5.5,0) node {\textcolor[rgb]{0,0,1}{$\cdot \sigma$}};
		\draw (3,2.2) node {$\mathcal{P}_U$};
		\draw (4.71,-0.6) node {$X$};
	...
	\end{center}
	\end{minipage}
	Figure 1: Push-forward of a horizontal tangent vector $X$ with constant section (left) and general section(right).
\end{figure}
...
\ea
for all $p \in \mathcal{P}$ and $g \in G$. Recall that a connection has a 1:1 correspondence to a parallel transport, as presented in \cite[\S 5.8, page 286ff.]{Hamilton}. This means corresponding to a piece-wise smooth base curve $\alpha: [a, b] \to M$ ($a < b \in \mathbb{R}$) with $\alpha(a) = x$ we have a \textbf{parallel transport along $\alpha$} as a map $\mathrm{PT}_\alpha^{\mathcal{P}}: \mathcal{P}_x \to \mathcal{P}_{\alpha(b)}$ satisfying
\ba\label{PTProp1}
\mathrm{PT}^{\mathcal{P}}_{\alpha*\alpha^\prime}
&=
\mathrm{PT}^{\mathcal{P}}_{\alpha^\prime}
	\circ \mathrm{PT}_\alpha^{\mathcal{P}},\\
\mathrm{PT}_{\alpha^-}^{\mathcal{P}}
&=
\mleft( \mathrm{PT}_\alpha^{\mathcal{P}} \mright)^{-1},\label{PTProp2}
\ea
especially parallel transport is a diffeomorphism between the fibres, where $\alpha^\prime$ is just another similarly defined base curve with $\alpha^\prime(0)= \alpha(1)$, $\alpha*\alpha^\prime$ their concatenation ($\alpha$ coming first), and $\alpha^-$ denotes $\alpha$ traversed backwards. The second equation could also be replaced with the condition that the parallel transport along a constant curve is the identity. Moreover, Eq.\ \eqref{ClassicalSymmetryOfHorizontalDistr} integrates to
\ba\label{ClassicalSymmetryofPTs}
\mathrm{PT}_\alpha^{\mathcal{P}}(p \cdot g)
&=
\mathrm{PT}_\alpha^{\mathcal{P}}(p) \cdot g.
\ea
Thinking of the associated $\mathcal{G}$-action, $g$ is an element of $\mathcal{G}_x$ such that the right hand side is in general not well-defined anymore due to the fact that $\mathrm{PT}_\alpha^{\mathcal{P}}(p) \in \mathcal{P}_{\alpha(b)}$; it is well-defined if interpreting $g$ as a constant section of $\mathcal{G} = M \times G$, denoted by $\widetilde{g} \in \Gamma(\mathcal{G})$ for bookkeeping reasons. The left hand side uses $\widetilde{g}_{\alpha(a)} = (x, g)$, the right hand side $\widetilde{g}_{\alpha(b)} = (\alpha(b), g)$, and by Ex.\ \ref{ex:TrivialLGBAction} we rewrite the $G$-action to a $\mathcal{G}$-action:
\bas
p \cdot g
&=
p \cdot (x, g)
...
\mathrm{PT}_\alpha^{\mathcal{P}}(p) \cdot g
&=
\mathrm{PT}_\alpha^{\mathcal{P}}(p) \cdot (\alpha(b), g)
=
\mathrm{PT}_\alpha^{\mathcal{P}}(p) \cdot \widetilde{g}_{\alpha(b)}.
\eas

Now we equip $\mathcal{G} \stackrel{\pi_{\mathcal{G}}}{\to} M$ with its \textbf{canonical flat connection} $\mathrm{H}\mathcal{G} \coloneqq \pi^*_{\mathcal{G}}\mathrm{T}M$ which induces a parallel transport $\mathrm{PT}_\alpha^{\mathcal{G}}: \mathcal{G}_{x} \to \mathcal{G}_{\alpha(b)}$ with $\mathrm{PT}_\alpha^{\mathcal{G}}(x, g) = ( \alpha(b), g)$, especially $\mathrm{PT}_\alpha^{\mathcal{G}}\mleft( \widetilde{g}_x \mright) = \widetilde{g}_{\alpha(b)}$. In total, we can rewrite Eq.\ \eqref{ClassicalSymmetryofPTs} to
\bas
...
\eas
This opens a gateway to define Eq.\ \eqref{ClassicalSymmetryofPTs} on general principal $\mathcal{G}$-bundles. That is, now let $\mathcal{P}$ be again a general principal $\mathcal{G}$-bundle. Fix any horizontal distribution on $\mathcal{G}$ inducing a parallel transport $\mathrm{PT}_\alpha^{\mathcal{G}}$, then a connection on $\mathcal{P}$ should be equivalent to a parallel transport $\mathrm{PT}_\alpha^{\mathcal{P}}$ on $\mathcal{P}$ satisfying Eq.\ \eqref{PTProp1}, \eqref{PTProp2} and
\ba
\mathrm{PT}_\alpha^{\mathcal{P}}(p \cdot g)
&=
\mathrm{PT}_\alpha^{\mathcal{P}}(p) \cdot \mathrm{PT}_\alpha^{\mathcal{G}}(g)
\ea
for all $p \in \mathcal{P}_x$ and $g \in \mathcal{G}_x$. The right hand side is now well-defined since both, $\mathrm{PT}_\alpha^{\mathcal{P}}(p)$ and $\mathrm{PT}_\alpha^{\mathcal{G}}(g)$, are elements of $\mathcal{P}_{\alpha(b)}$ and $\mathcal{G}_{\alpha(b)}$, respectively. We now want to derive its infinitesimal analogue similar to Eq.\ \eqref{ClassicalSymmetryOfHorizontalDistr}; for this we want to use Thm.\ \ref{thm:DiffOfLGBAction}.
