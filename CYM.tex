...
\mu_{\mathcal{G}}^{\mathrm{tot}}
	+ \delta \lambda ~.
\ees
Thus, $\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}$ stays closed, and hence it is multiplicative. That it satisfies the generalised Maurer-Cartan equation w.r.t.\ $\zeta^\lambda$ is a straight-forward but a bit lengthy calculation, which is why the keep this as an exercise for the reader; for the infinitesimal, i.e.\ in the connected case by Thm.\ \ref{thm:GenMCEq}, see \cite{My1stpaper, MyThesis}.
\end{proof}
