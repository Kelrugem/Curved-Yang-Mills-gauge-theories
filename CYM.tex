...
Let $\mathcal{G} \stackrel{\pi_{\mathcal{G}}}{\to} M$ be an LGB over a smooth manifold $M$, $\mathrm{H}\mathcal{G}$ an Ehresmann connection on $\mathcal{G}$ satisfying the infinitesimal compatibility condition \eqref{CondKruemmungmitBLAB} w.r.t.\ a $\zeta \in \Omega^2\mleft( M; \mathcal{g} \mright)$. Then
\bas
...
%\mright)
\eas
Let us denote $X^h \coloneqq \pi_h(X)$ and $Y^h \coloneqq \pi_h(Y)$, where $X$ and $Y$ are elements of $\mathfrak{X}(\mathcal{P})$, and we rewrite
\bas
\mathcal{r}_{\sigma*}(X)
&=
\mathrm{D}r_{\sigma}(X)
