...
Viewing the LGB $\mathcal{G}$ as a principal $\mathcal{G}$-bundle, one can show that Yang-Mills connections are a special type of Ehresmann connection.

\begin{lemmata}{Yang-Mills connections as a special Ehresmann connection}{YangMillsConnAnEhresmann}
Let $\mathcal{G} \to M$ be an LGB over a smooth manifold $M$, and $\mathrm{H}\mathcal{G}$ a horizontal distribution on $\mathcal{G}$. Then $\mathrm{H}\mathcal{G}$ is an (Ehresmann) connection on $\mathcal{G}$ as principal bundle if Eq.\ \eqref{CondSGleichNullLAB} is satisfied.
\end{lemmata}

\begin{proof}
\leavevmode\newline
This can be quickly answered by recalling how the parallel transport behaves; recall that Def.\ \ref{def:FinallyTheConnection} comes from Eq.\ \eqref{OiTHatIsHowWeFormulateHorizSymmetry} which itself stems from Eq.\ \eqref{PTHomomNEw}. Eq.\ \eqref{CondSGleichNullLAB} means that $\nabla^{\mathrm{YM}}$ is a Lie bracket derivation; integrated this means that the parallel transport $\mathrm{PT}^{\mathcal{G}}$ of $\mathcal{G}$ (omitting the notation of the involved curve) is a homomorphism, \textit{i.e.}\
\bas
\mathrm{PT}^{\mathcal{G}}(gq)
&=
\mathrm{PT}^{\mathcal{G}}(g) ~ \mathrm{PT}^{\mathcal{G}}(q)
\eas
for all $(g, q) \in \mathcal{G} * \mathcal{G}$. As a principal bundle, $\mathcal{G}$ equipped with its Yang-Mills connection satisfies Eq.\ \eqref{PTHomomNEw}, and thus $\mathrm{H}\mathcal{G}$ is an Ehresmann connection.
\end{proof}

\begin{remarks}{Base manifold a horizontal leave}{baseManifoldHoriz}
Observe that the proof's argument implies that a parallel transported neutral element is again the neutral element of the corresponding final fibre. Thus, $M$ naturally embedded via $e$, the neutral element section, can be viewed as a horizontal leave of $\mathrm{H}\mathcal{G}$ in this case.
\end{remarks}

Let us provide canonical examples of Yang-Mills connections.
...
F
&\coloneqq
\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A \circ \mleft( \pi^{\mathrm{H}\mathcal{G}}, \pi^{\mathrm{H}\mathcal{G}} \mright)
	+ \pi^!\zeta,
\eas
where $\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}}$ is the exterior covariant derivative related to the pullback connection $\pi^*\nabla^{\mathrm{YM}}$, and $\pi^{\mathrm{H}\mathcal{G}}: \mathrm{T}\mathcal{G} \to \mathrm{H}\mathcal{G}$ is the canonical projection onto the Yang-Mills connection $\mathrm{H}\mathcal{G}$. That is,
\bas
F
&=
\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A \mleft( \pi^{\mathrm{H}\mathcal{G}}(X), \pi^{\mathrm{H}\mathcal{G}}(Y) \mright)
	+ \mleft(\pi^*\zeta\mright) \bigl( \mathrm{D}\pi(X), \mathrm{D}\pi(Y) \bigr)
\eas
for all $X, Y \in \mathfrak{X}(\mathcal{P})$.
\end{definitions}

Of course, we expect certain properties, naturally generalized by our previous discussion; however, in order to prove these we need to discuss the Darboux derivative w.r.t.\ Yang-Mills connections.\footnote{Many of the following properties can actually be showed w.r.t.\ every Ehresmann connection, not just the Yang-Mills ones.} Let us start with an auxiliary result and then a Leibniz rule; especially recall all the discussion around the modified push-forward via right-translations $\mathcal{r}$, Def.\ \ref{def:WeModTheRIghtPush}.
...
Let $\mathcal{G} \stackrel{\pi_{\mathcal{G}}}{\to} M$ be an LGB over a smooth manifold $M$, and $\mathrm{H}\mathcal{G}$ a Yang-Mills connection on $\mathcal{G}$. Then we have
\bas
\Delta(\sigma \tau)
&=
...
\eas
for all $x \in M$, $X \in \mathrm{T}_xM$ and $\sigma, \tau \in \Gamma(\mathcal{G})$, where we used that $\pi_{\mathcal{G}} \circ \sigma = \mathds{1}_M$. Viewing $\mathcal{G}$ as a principal $\mathcal{G}$-bundle we know by Lemma \ref{lem:YangMillsConnAnEhresmann} that $\mathrm{H}\mathcal{G}$ is an Ehresmann connection so that we can apply Cor.\ \ref{cor:ModifiedRightPushyCommutesWithProj}. Together with recalling Def.\ \ref{def:FundVecs} we derive
\bas
\Delta(\sigma\tau)_x(X)
&=
...
Now recall Remark \ref{rem:baseManifoldHoriz}; Lemma \ref{lem:YangMillsConnAnEhresmann} implies for the neutral element section $e$ that
\bas
\Delta e
&=
0,
...
Thence, let us focus on proving the first bullet point.
\end{proof}
