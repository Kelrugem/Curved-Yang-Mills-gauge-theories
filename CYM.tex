...
&=
\sAd_{\sigma^H}(A) 
	+ \mleft(\pi^*\Delta\mright)\mleft( \sigma^H \mright),
\eas
where $\sigma^H \in C^\infty(\mathcal{P}; \mathcal{G})^{\mathcal{G}}$ is defined as in Prop.\ \ref{prop:GaugeTrafoAsBundleIsomIsASectionOfConjugationMaps}.
\end{theorems}

\begin{proof}
\leavevmode\newline
\indent $\bullet$ First of all observe that $H^!A \in \Omega^1(\mathcal{P}; H^*\pi^*\mathcal{g})$, and trivially $H^*\pi^*\mathcal{g} \cong (\pi\circ H)^*\mathcal{g} = \pi^*\mathcal{g}$. We will make use of this in the following without further mentioning it. We then have
\bas
...
\eas
where we wrote $A = \mleft( p, \widehat{A}_p \mright)$ with $\widehat{A} \coloneqq \mathrm{pr}_2 \circ A$ ($\mathrm{pr}_2: \pi^*\mathcal{g} \to \mathcal{g}$ the projection onto the second component), and thus
\bas
...
p \cdot \sigma^H_p
=
\Phi\mleft(p, \sigma^H_p\mright)
=
\Bigl( \Phi \circ \mleft( \mathds{1}_{\mathcal{P}}, \sigma^H \mright) \Bigr)(p)
\eas
for all $p \in \mathcal{P}$, where $\Phi$ is the right $\mathcal{G}$-action on $\mathcal{P}$.
\end{proof}
