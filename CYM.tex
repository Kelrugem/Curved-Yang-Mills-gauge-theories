...
\ea
for all $(p, g) \in N*\mathcal{G}$, similarly for $\nu$; here we understand the LABs of LGBs as left-invariant vector fields as in Cor.\ \ref{cor:LeftInvVfToLAB}. Observe that we have in general for $\eta \Gamma(f^*\mathcal{g}) \in \cong L(f^*\mathcal{G})$, using the definition of $\rho$,
