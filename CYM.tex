...
f X_T
	...
	\mright)
\eas
where we viewed the last summand in Eq.\ \eqref{TotalDerivativeOfTMWithLeibniz} as a separate vector which is clearly vertical, so that its image under $S$ is a tangent vector along the zero vector field $0$. Recalling that the vertical bundle of $\mathrm{T}M$ is canonically isomorphic to $\pi_{\mathrm{T}M}^*\mathrm{T}M$ and that $\mathrm{T}M$ is embedded into $\mathrm{TT}M$ via the zero section, one can think of the last term as $\mathrm{d}f \boldsymbol{\cdot} X$.
\end{remarks}
