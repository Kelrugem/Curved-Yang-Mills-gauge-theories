...
\begin{enumerate}
	...
	and this can be viewed as $\mathcal{G}$-equivariance (under the trivialization induced by $\phi_i$).
	...
	\item Due to the existence of $\varphi_i$ one does not need to claim upfront that $\mathcal{P}$ is a $G$-fibre bundle. However, for readability, we decided to structure the definition like this. We kept a similar style as for "classical" principal bundles as provided in \cite[\S 4.2, Def.\ 4.2.1, page 207f.]{Hamilton}.
...
We often refer to principal bundles related to trivial LGBs as \textbf{classical principal bundles}.
\end{examples}
...
Our main example will be the inner bundle of a classical principal bundle, recall Ex.\ \ref{ex:InnerLGBs}. 
...
\ea
for all $(p, h) \in \mathcal{P}^\prime * \mathcal{H} = \mleft(\pi^\prime\mright)^*\mathcal{H}$.

We speak of a \textbf{principal bundle isomorphism (over $f$)} if $H$ is a diffeomorphism.
\end{definitions}
...
\eas
for all $(q, g) \in \mathcal{P}*\mathcal{G}$, which is well-defined by similar arguments as before, especially because the inverses are maps over $f^{-1}$. To prove this, observe that there is a unique $(p, h) \in \mathcal{P}^\prime * \mathcal{H}$ such that $H(p) = q$ and $F(h) = g$ due to bijectivity.

$\bullet$ Lokal by H = Principal Chart, F = Id eine "Trivialisierung"
\end{remark}

\begin{corollaries}{Pullbacks of $\mathcal{G}$-principal bundles}{PullBacksArePrincToo}
Let $M, N$ be smooth manifolds, $f: N \to M$ a smooth map, $\mathcal{G} \to M$ an LGB, and $\mathcal{P} \to M$ a $\mathcal{G}$-principal bundle. Then 
\end{corollaries}
