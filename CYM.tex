...
\begin{definitions}{Left and right translation and conjugation, \newline \cite[\S 1.5, straightforward generalization of Def.\ 1.5.3, page 40]{Hamilton}}{LeftRightTranslationConjugation}
...
Additionally, we have the following examples, the third of which will be important in this paper. The notation will be as in Def.\ \ref{def:LRTranslations}.
...
(g,h) &\mapsto gh.
\eas
The left and right translations of Def.\ \ref{def:LeftRightTranslationConjugation} and \ref{def:LRTranslations} then align, and thus the smoothness concerns as mentioned in the last part of Rem.\ \ref{SmoothnessOfACtionTranslations} for right translations $r_g = R_g$ ($g \in \mathcal{G}$) do not arise.
\end{examples}
...
\eas
 ...
 In that case $e_{f(p)} = (f(p), e) \in M \times \mathcal{G}$, where $e$ on the right hand side is the neutral element of $G$. Furthermore, 
\end{examples}
