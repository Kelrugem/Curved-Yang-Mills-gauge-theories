...
&=
\pi^*\mleft(
	\nabla^{\mathrm{YM}}_{\mathrm{D}\pi\mleft(X^h\mright)}\Bigl( \mleft(\Delta \sigma \mright) \mleft(\omega^Y\mright)\Bigr)
\mright)
\\
&=
...
Collecting all these results we can write Eq.\ \eqref{TransformedClassicStrengthTerm} as
\bas
&
r_\sigma^*\mleft(
\mleft(\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) \mleft(\mathcal{r}_{\sigma*}\mleft(X^h\mright), \mathcal{r}_{\sigma*}\mleft(Y^h\mright)\mright)
...
\\&\hspace{1.5cm}
	+ r_{\sigma}^*\Biggl(
		\mleft( \pi^*\nabla^{\mathrm{YM}} \mright)_{X^h}\biggl( \mleft(\pi^! \Delta \sigma \mright) \mleft(Y^h\mright)\biggr)
		- \mleft( \pi^*\nabla^{\mathrm{YM}} \mright)_{Y^h}\biggl( \mleft(\pi^! \Delta \sigma \mright) \mleft(X^h\mright)\biggr)
\\&\hspace{3cm}
