...
and $\pi^{\mathrm{vert}}$ is then equivalent to $\mathrm{Dpr}_2 \in \Omega^1\mleft(\mathcal{G}; \mathrm{pr}_2^*\mathrm{T}G\mright)$. 

Now let us view $G$ as an LGB over a point $\{*\}$, then $\mathcal{G} = f^*G$, where $f: M \to \{*\}$. By Remark \ref{rem:PullbackTotMCForm} we then get
\bas
\mu^{\mathrm{tot}}_{\mathcal{G}}
&=
\mathrm{pr}_2^!\mu_G,
\eas
where $\mu_G$ is the Maurer-Cartan form of $G$, making use of the uniqueness of $\pi^{\mathrm{vert}}$ for $G \to \{*\}$.
%
...
Following Remark \ref{rem:PullbackTotMCForm}
\end{remarks}
