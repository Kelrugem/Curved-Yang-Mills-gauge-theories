...
Let $\mathcal{G} \to M$ be an LGB over a smooth manifold $M$, and $\mathrm{H}\mathcal{G}$ be a Yang-Mills connection on $\mathcal{G}$. Then $\mathrm{H}\mathcal{G}$ is an (Ehresmann) connection on $\mathcal{G}$ as principal bundle.
\end{lemmata}

\begin{proof}
\leavevmode\newline
This can be quickly answered by recalling how the parallel transport behaves; recall 
\end{proof}
