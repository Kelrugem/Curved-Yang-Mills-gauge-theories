...
\mathrm{T}_p N
\eas
for all $(p, \nu) \in f^*\mathcal{g}$, and thus describes a vector bundle morphism $f^*\mathcal{g} \to \mathrm{T}N$. By Eq.\ \eqref{InvarianceOffUnderGAction} we have
\bas
\mathrm{D}f \circ \rho
&=
...
=
\mleft.\mathrm{D}\mleft(f^*\pi\mright)\mright|_{f^*\mathcal{g}}
= 0,
\eas
making use of that $f^*\mathcal{g}$ is generated by $\mathcal{g}$ and that $\mathcal{g}$ consists of vertical vectors.
\end{proof}
