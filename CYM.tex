...
\eas
By Remark \ref{AlternativePrincBdlDef} the first arrow is a diffeomorphism and thence closed. The second arrow is the inclusion, an embedding, and $\mathrm{dim}\mleft(\pi^*\mathcal{P}\mright) = \mathrm{dim}(\mathcal{P} \times \mathcal{P}) - \mathrm{dim}(M)$, where $G$ is the Lie group as structural fibre of $\mathcal{P}$; if $\mathrm{dim}(M) \neq 0$, then $\pi^*\mathcal{P}$ is a closed embedded submanifold (for $\mathrm{dim}(M) = 0$ this follows trivially).
\end{remarks}
