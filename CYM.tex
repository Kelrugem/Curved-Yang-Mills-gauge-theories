...
For another important example recall that there is the notion of associated fibre bundles; following and stating the results of \cite[\S1, Construction 1.3.8, page 20]{mackenzieGeneralTheory} and \cite[\S 4.7, page 237ff.; see also Rem.\ 4.7.8, page 242f.]{Hamilton}: Let $P \stackrel{\pi_P}{\to} M$ be a principal bundle with structural Lie group $G$, $N$ a smooth manifold, and $\Psi$ a smooth left $G$-action denoted by
\bas
G \times N &\to N,\\
...
\begin{center}
	...
	\end{tikzcd}
\end{center}
such that the map to the equivalence classes $P \times N \to P \times_\Psi N$ is a smooth surjective submersion,
...
\bas
\mleft(\mathrm{Dpr}_1, \mathrm{Dpr}_2 \mright) \circ \underbrace{\mathrm{pr}_2^!\pi^{\mathrm{vert}}}_{\mathclap{ = \mathrm{pr}_2^*\pi^{\mathrm{vert}} \circ \mathrm{Dpr}_2 }}
&= 
\mleft( 0, \pi^{\mathrm{vert}} \circ \mathrm{Dpr}_2 \mright).
\eas 
...
\begin{remarks}{Pullback situation}{PullbackTotMCForm}
...
\bigl(x, L_g(q)\bigr)
\eas
for all $(x, g), (x, q) \in f^*\mathcal{G}$, where the left-translation on the left and right hand side are the ones of $f^*\mathcal{G}$ and $\mathcal{G}$, respectively. Thence,
\bas
L_{(x, g)}
&=
\mleft(
...
\mright)
\mright|_{\mathrm{V}_{(x, g)}\mleft( f^*\mathcal{G} \mright)}
\\
&=
\mleft.
\mleft(
	0, 
	\mleft( \mu_{\mathcal{G}} \mright)_{\mathrm{pr}_2(x, g)} \circ \mathrm{D}_{(x,g)}\mathrm{pr}_2
\mright)\mright|_{\mathrm{V}_{(x, g)}\mleft( f^*\mathcal{G} \mright)},
%\\
%&=
%\mleft(
	%\mathrm{Dpr}_1, 
...
%\mright),
\eas
making use of that $f^*\mathcal{G}$ is an embedded submanifold of $N \times \mathcal{G}$ and that $L_{(x, g)}: \mleft(f^*\mathcal{G}\mright)_x \to \mleft(f^*\mathcal{G}\mright)_x$ so that $\mathrm{D}_{(x, g)}L_{(x, g)^{-1}}:\mathrm{V}_{(x, g)}\mleft( f^*\mathcal{G} \mright) \to \mathrm{V}_{(x, e_x)}\mleft( f^*\mathcal{G} \mright) = f^*\mathcal{g}_{(x, e_x)}$, where $e_x$ is the neutral element of $\mathcal{G}_x$.
Altogether we get
\bas
\mleft(\mu_{f^*\mathcal{G}}^{\mathrm{tot}}\mright)_{(x, g)}
&=
\mathrm{D}_{(x, g)} L_{(x, g)^{-1}} \circ \mleft.\mathrm{pr}_2^!\pi^{\mathrm{vert}}\mright|_{(x, g)}
\\
&=
\mleft(
	...
	\mleft( \mu_{\mathcal{G}} \mright)_{\mathrm{pr}_2(x, g)} \circ \mathrm{D}_{(x,g)}\mathrm{pr}_2
\mright)
	\circ \mleft.\mathrm{pr}_2^!\pi^{\mathrm{vert}}\mright|_{(x, g)}
\\
&=
\mleft(
	0, 
...
\eas
for all $p \in \mathcal{P}$, where $\mathrm{pr}_2: \pi^*\mathcal{G} \to \mathcal{G}$ is the projection onto the second component. In essence, we drop the notation of $\mathrm{pr}_2$, as if we view $\sigma$ as a map $\mathcal{P} \to \mathcal{G}$ of fibre bundles over $\pi$; recall Subsection \ref{BasicNotations}.
...
Using this, we can finally formulate the gauge transformations of connection 1-forms; for this recall Cor.\ \ref{cor:PullBacksArePrincToo} and the Darboux derivative, Def.\ \ref{def:DarbouxDerivativeOnLGBs}.

\textbf{Mit PullbackDarboux umschreiben?}

\begin{theorems}{Gauge transformations of connection 1-forms}{GaugeTrafoOfGaugeBoson}
Let $\mathcal{G} \to M$ be an LGB over a smooth manifold $M$ and $\mathcal{P} \stackrel{\pi}{\to} M$ a principal $\mathcal{G}$-bundle, and let $\mathrm{H}\mathcal{G}$ be a horizontal distribution on $\mathcal{G}$ and $A \in \Omega^1(\mathcal{P}; \pi^*\mathcal{g})$ be a connection 1-form on $\mathcal{P}$. Furthermore, let $H \in \sAut(\mathcal{P})$. Then $H^!A$ is a connection 1-form on $f^*\mathcal{P}$ and we have
\bas
H^!A
&=
\sAd_{\sigma^H}(A) 
	+ \Delta\mleft( \mathrm{pr}_2 \circ \sigma^H \mright),
\eas
where $\sigma^H \in C^\infty(\mathcal{P}; \mathcal{G})^{\mathcal{G}}$ as in Prop.\ \ref{prop:GaugeTrafoAsBundleIsomIsASectionOfConjugationMaps}, and $\mathrm{pr}_2: \pi^*\mathcal{G} \to \mathcal{G}$ is the projection onto the second component. If we view $\sigma^H$ as a map $\mathcal{P} \to \mathcal{G}$ over $\pi$ for the second summand as discussed in Subsubsection \ref{BasicNotations}, then we may also write shortly
\bas
H^!A
&=
\sAd_{\sigma^H}(A) 
