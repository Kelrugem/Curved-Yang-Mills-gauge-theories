...
Let $M, N$ be smooth manifolds, $\mathcal{G} \stackrel{\pi}{\to} M$ an LGB over $M$ and $f: N \to M$ a smooth map. Then any right (or left) $\mathcal{G}$-action $\Phi: N * \mathcal{G} = f^*\mathcal{G} \to N$ on $N$ induces a $\mathcal{g}$-action $\rho$ on $N$ by 
\bas
\rho
&\coloneqq
...
\eas
for all $(p, g) \in N* \mathcal{G}$ and $t \in \mathbb{R}$,
where we write in general $\eta_{(p, e_x)} = \mleft(p, \xi_{p}\mright)$ with $\xi_{p} \in \mathcal{g}_x$ ($x \coloneqq f(p)$), and the exponential $\e$ is the one of $\mathcal{G}$. But we also have similarly
\bas
...
It is clear that one has a local version of Eq.\ \eqref{ActionLieAlgebroidButNonTrivial}. In fact, as one sees in the proof of Lemma \ref{lem:LGBInduceLABAction}, the argument for Eq.\ \eqref{ActionLieAlgebroidButNonTrivial} works pointwise in $M$, so, we may have a homomorphism of Lie algebras
\bas
\mathcal{g}_{x} &\to \Gamma\mleft(\mleft.\mathrm{T}N\mright|_{f^{-1}(\{x\})}\mright),\\
\nu &\mapsto \mleft[ f^{-1}(\{x\}) \ni p \mapsto \rho(p, \nu) \mright]
\eas
for all $x \in M$. Due to a possible lack of manifold structure on $f^{-1}(\{x\})$ we did not restrict to $x\in M$ to avoid technical difficulties. However, $f$ will be the projection of a bundle later, and in that case the fibre $f^{-1}(\{x\})$ is an embedded submanifold of $N$. As argued in Remark \ref{rem:LocalLGBAction}, the $\mathcal{G}$-action is a $\mathcal{G}_x$-action restricted on $f^{-1}(\{x\})$. By what we know about Lie group actions we immediately know that we have the aforementioned Lie algebra homomorphism, and so also Eq.\ \eqref{ActionLieAlgebroidButNonTrivial}. Furthermore, the LAB action has then also values in the vertical bundle by Eq.\ \eqref{LABActionAlongFibres}.
\end{remarks}

An important example will be fundamental vector fields which we will introduce later. Another example are LAB representations induced by LGB representations.

\begin{definitions}{LAB representations, \cite[\S 3.3, Def.\ 3.3.13, page 107]{mackenzieGeneralTheory}}{LABReps}
Let $\mathcal{g} \stackrel{\pi}{\to} M$ be an LAB over a smooth manifold $M$, $V \stackrel{p}{\to} M$ be a vector bundle, and assume that we have a $\mathcal{g}$-action on $V$, $\psi: p^*\mathcal{g} \to \mathrm{T}V$. Then we say that $\psi$ is a \textbf{$\mathcal{g}$-representation on $V$} if it has values in the set of linear vector fields on $V$, that ist,
\bas
\psi(\nu)_{\alpha v + \beta w}
&=
\alpha 
