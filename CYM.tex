...
\eas
for all $x \in M$, $X \in \mathrm{T}_xM$ and $\nu \in \Gamma(\mathcal{g})$, is a vector bundle connection on $\mathcal{g}$, where $t \in \mathbb{R}$.
\end{propositions}
...
Similar to Diagram \eqref{DoubleTangentAsDiagram} we have the double vector bundle
\begin{center}
	\begin{tikzcd}
...
\Bigl( S_{\mathcal{G}} \bigl( \mathrm{D}_x\nu (X) \bigr) \Bigr)
\\
&=
...
The projection of $\mathcal{g}\to M$ is canonically the restriction of $\pi_{\mathrm{T}\mathcal{G}}$, hence we can conclude that $\mleft.\nabla^{\mathcal{G}}_X \nu\mright|_x \in \mathrm{V}\mathcal{g}$

Furthermore, due to the assumption that $e$ is a parallel section we know that $\mu_{\mathcal{G}}^{\mathrm{tot}}\bigl( \mathrm{D}_x e(X) \bigr) = 0$, thus,
\bas
\pi_{\mathrm{TT}\mathcal{G}}\mleft( \mleft.\nabla^{\mathcal{G}}_X \nu\mright|_x \mright)
&=
0.
\eas
...
\mleft.\nabla^{\mathcal{G}}_X \nu\mright|_x
&\in
\mathrm{V}_{e_x}\mathrm{T}\mathcal{G},
\eas
where we make use of the beginning of Remark \ref{rem:BothLinearStructuresTheSameOnTheVerticalBundle}, that is we have a canonical embedding of $\mathcal{G}$ into $\mathrm{T}\mathcal{G}$; additionally, via the neutral element section $e$ of $\mathcal{G}$, we have a canonical embedding of $M$ into $\mathcal{G}$ and thus into $\mathrm{T}\mathcal{G}$. Despite the notation in Remark \ref{rem:BothLinearStructuresTheSameOnTheVerticalBundle}, we will still denote the embedded manifold $M$ simply as $M$. Therefore
\bas
\nabla^{\mathcal{G}} \nu
&\in
\mleft.\mathrm{V}\mathrm{T}\mathcal{G}\mright|_{M}
\eas
\end{proof}
...
\eas
called the \textbf{tangent lift of $X$}.
\end{remarks}
...
\\
&=
\lambda \boldsymbol{\cdot} \mathrm{D}L(X) \RPlus \kappa \mathrm{D}L(Y)
\eas
for all $\lambda, \kappa \in \mathbb{R}$. Hence we achieve linearity with respect to both vector bundle structures of $\mathrm{TT}M$.
\end{remarks}

\begin{remarks}{Alignment of both vector bundle structures on $\mathrm{TT}M$ on the restricted vertical bundle, \newline \cite[\S 9.6, Thm.\ 9.6.1, page 363; but without proof]{mackenzieGeneralTheory}}{BothLinearStructuresTheSameOnTheVerticalBundle}
