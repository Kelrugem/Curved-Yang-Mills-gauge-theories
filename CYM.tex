...
\\
&=
\mathrm{Ad}_{\sigma^{-1}} \circ \Delta \mleft( \sigma \e^\nu \mright)
	+ \Delta \mleft( \sigma^{-1} \mright)
\\
&=
\mathrm{Ad}_{\sigma^{-1}} \circ \mleft( \mathrm{Ad}_{\e^{-\nu}} \circ \Delta \sigma + \Delta e^\nu \mright)
	- \mathrm{Ad}_\sigma \circ \Delta \sigma
\eas
\end{proof}
...
\leavevmode\newline
The idea of the proof is similar to the proof of a statement about when two multiplicative 2-forms are equal, see \textit{e.g.}\ \cite[\S 3, Cor.\ 3.4]{bursztyn2004integration}. That is, we now first show that 
\bas
...
\mleft.\mleft(\mathrm{d}^{\pi_{\mathcal{G}}^*\nabla^{\mathcal{G}}} \mu_{\mathcal{G}}^{\mathrm{tot}}
	...
	- \sAd_{g^{-1}} \circ \mleft.\mleft(\pi_{\mathcal{G}}^! \zeta\mright)\mright|_g
\eas
is constant along the fibres of $\mathcal{G}$, that is, constant along the flows of vertical tangent vectors. For a given $g \in \mathcal{G}_x$ ($x \in M$) take any curve $\gamma: I \to \mathcal{G}_x$ ($I$ an open interval of $\mathbb{R}$ containing 0) so that $\gamma(0) = g$.
Recall Cor.\ \ref{cor:TLGBAsLGB}
\end{proof}
