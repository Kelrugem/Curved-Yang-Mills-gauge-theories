...
\begin{propositions}{Field redefined Yang-Mills connection a Yang-Mills connection}{YMStaysYM}
Given the context of Def.\ \ref{def:FieldRedefsAsDefIntegrated}, the field redefinition of $\mu_{\mathcal{G}}^{\mathrm{tot}}$ is a Yang-Mills connection w.r.t.\ $\widetilde{\zeta}^\lambda$.
\end{propositions}
...
Let $\mathcal{G} \to M$ be a trivial LGB over a smooth manifold $M$; furthermore let $\mu_{\mathcal{G}}^{\mathrm{tot}}$ be a Yang-Mills connection on $\mathcal{G}$. Then there is a $\lambda \in \Omega^1(M; \mathcal{g})$ so that $\mu_{\mathcal{G}}^{\mathrm{tot}, \lambda}$ is flat.
\end{conjectures}
