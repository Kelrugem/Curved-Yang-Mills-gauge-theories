...
H^!A
&=
\sAd_{\mathrm{pr}_2\circ\mleft(\sigma^H\mright)^{-1}}(A) 
...
\begin{proof}
\leavevmode\newline
For $\sigma \in C^\infty \mleft( \mathcal{P}_U; \mathcal{G}_U \mright)^{\mathcal{G}_U} \subset \Gamma\mleft( \pi^*\mathcal{G}_{U} \mright)$ observe that $s^*\sigma$ is by construction a section of $\mathcal{G}_U$ due to using $s^*\pi^*\mathcal{G}_U \cong (\pi \circ s)^*\mathcal{G}_U = \mathds{1}_U^*\mathcal{G}_U \cong \mathcal{G}_U$; we will make use of these natural isomorphisms in the following without further mention, especially we will view pullback sections as sections along a map, essentially dropping the base point component here and there in order to avoid introducing extra symbols for needed isomorphisms (recall Subsection \ref{BasicNotations}). For the supposed inverse we have similarly that $\sigma^\tau$ is a section of $\Gamma(\pi^*\mathcal{G}_U)$; then observe, using Lemma \ref{lem:SectionsNowInduceIsomToLGBsNotNecTriv},
\bas
\sigma_{p \cdot g}^\tau
&=
...
\eas
for all $(p, g) \in \mathcal{P}_U * \mathcal{G}_U$, thus, $\sigma^\tau \in C^\infty \mleft( \mathcal{P}_U; \mathcal{G}_U \mright)^{\mathcal{G}_U}$. Then first of all derive
\bas
\Phi^{-1}_{s} \circ s
&=
e|_U,
\eas
where $e$ is the section of the neutral element of $\mathcal{G}$; this is a direct implication of the definition of $\Phi_s$. Then
\bas
\mleft.s^*\sigma^\tau\mright|_x
&=
\sigma^\tau_{s_x}
=
...
\tau_x
\eas
for all $x \in M$ and $\tau \in \Gamma(\mathcal{G}_U)$, and
\bas
\sigma^{s^*\sigma}_p
&=
\mleft(p, c_{\mleft(\Phi^{-1}_{s}(p)\mright)^{-1}} \mleft( \mleft. s^*\sigma \mright|_{\pi(p)} \mright) \mright)
\eas
for all $p \in \mathcal{P}_U$ and $\sigma \in C^\infty \mleft( \mathcal{P}_U; \mathcal{G}_U \mright)^{\mathcal{G}_U}$
\end{proof}
...
\sigma^{H_{ji}}
&=
\mathcal{c}_{\mleft(\Phi^{-1}_{s_j}\mright)^{-1}} \mleft( \pi^*\sigma_{ji} \mright)
\eas
