...
\bas
f^{-1}(U) * \mleft.\mathcal{G}\mright|_{U} \coloneqq \mleft( f|_U\mright)^*(\mathcal{G}|_U) = \mleft.(N*\mathcal{G})\mright|_{f^{-1}(U)} &\to f^{-1}(U).
\eas
...
\bas
\mathcal{P}_{U_i \cap U_j} &\to \mathcal{P}_{U_i \cap U_j},\\
...
\begin{lemmata}{The conjugation of the canonical LAB connection a Yang-Mills connection}{FieldRedefViaLGBSection}
Let $\mathcal{G} \stackrel{\pi_{\mathcal{G}}}{\to} M$ be an LGB over a smooth manifold $M$, and $\mathrm{H}\mathcal{G}$ an Ehresmann connection on $\mathcal{G}$. Then we have
...
\mleft[ \mathrm{d}^{\nabla^{\mathrm{YM}}} \Delta \sigma + \frac{1}{2} \mleft[ \Delta \sigma \stackrel{\wedge}{,} \Delta\sigma \mright] + \zeta - \mathrm{Ad}_{\sigma^{-1}} \circ \zeta, ~ \nu \mright]_{\mathcal{g}}
&=
0
\ea
for all $\nu \in \Gamma(\mathcal{g})$.
...
R_{\widetilde{\nabla}^\sigma} 
&=
\mleft[ \mathrm{d}^{\nabla^{\mathrm{YM}}} \Delta \sigma + \frac{1}{2} \mleft[ \Delta \sigma \stackrel{\wedge}{,} \Delta\sigma \mright] + \zeta, \cdot \mright]_{\mathcal{g}}.
\eas
...
\mleft. \pi^*\mleft( \nabla^{\mathcal{G}}_\omega \nu \mright) \mright|_p.
\eas
Concluding the proof, by the definition of pullback connections we could also write
\bas
A\bigl( \mleft[ X, \widetilde{\nu} \mright] \bigr)
&=
\mleft( \pi^*\nabla^{\mathcal{G}} \mright)_{X}(\pi^*\nu).
...
\mleft( g, \mathrm{Ad}_{g^{-1}} \circ \zeta_x \circ \mathrm{D}_g \pi_{\mathcal{G}} \mright)
\eas
...
\begin{remark}
\leavevmode\newline
The structure on the right hand side is again due to denoting the base point in pullback bundles; one may decide to drop the base point for simplicity in the notation.
\end{remark}
...
\coloneqq
\sAd_{g^{-1}} \circ \mleft.\mleft(\pi_{\mathcal{G}}^! \xi\mright)\mright|_g
...
\leavevmode\newline
The idea of the proof is similar to the proof of a statement about when two multiplicative 2-forms\footnote{See Rem.\ \ref{rem:SimplicialDifferentialStuff}; multiplicative is related to closedness w.r.t.\ a differential.However, we will not need this general notion.} are equal, see \textit{e.g.}\ \cite[\S 3, Cor.\ 3.4]{bursztyn2004integration}. 

$\bullet$ That is, we now first discuss under what conditions 
\bas
...
&=
\mathrm{d}^{\gamma(t)^*\pi_{\mathcal{G}}^*\nabla^{\mathcal{G}}} \mleft( \gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}}\mright)
=
\mathrm{d}^{\mleft(\pi_{\mathcal{G}}\circ \gamma(t)\mright)^*\nabla^{\mathcal{G}}} \mleft( \gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}}\mright)
=
\mathrm{d}^{\nabla^{\mathcal{G}}} \mleft( \gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}}\mright),
\eas
using the definition of pullback vector bundle connections; alternatively see the explicit calulations in \cite[Appendix, Prop.\ A.1, (A.1)]{My1stpaper} or \cite[Appendix, Prop.\ A.1.1, (A.2)]{MyThesis}. We also have
\bas
...
\\
&\hspace{1cm}=
\mleft[ \mleft(\gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}}\mright)_{x}(X), 
\mleft(\gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}}\mright)_{x}(Y) \mright]_{\mathcal{g}_{x}}
\\
&\hspace{1cm}=
\mleft( \frac{1}{2}
	\mleft[ \gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}} \stackrel{\wedge}{,}
	\gamma^!\mu_{\mathcal{G}}^{\mathrm{tot}} \mright]_{\mathcal{g}}
\mright)_x(X,Y)
\eas
for all $x \in M$ and $X, Y \in \mathrm{T}_xM$.
\end{proof}
...
r_\sigma^*\mleft(
{\oversortoftilde{
	\mleft[ \mleft(\pi^! \Delta \sigma \mright) \mleft(X^h \mright), \mleft(\pi^! \Delta \sigma \mright)\mleft(Y^h \mright) \mright]
}}
\mright),
\eas
...
&\hspace{0.5cm}
=
r_\sigma^*\Biggl(\mleft( \pi^*\nabla^{\mathrm{YM}} \mright)_{\mathcal{r}_{\sigma*}\mleft(X^h\mright)}\biggl( \mleft(\pi^! \Delta \sigma \mright) \mleft(Y^h\mright)\biggr)\Biggr)
	+ r_\sigma^*\mleft(
	\mleft[ \mleft(\pi^! \Delta \sigma \mright) \mleft(X^h \mright), \mleft(\pi^! \Delta \sigma \mright)\mleft(Y^h \mright) \mright]
\mright)
\eas
...
\bas
\mathrm{D}\pi\mleft( \mathcal{r}_{\sigma*}\mleft(Y^h\mright) \mright)
&=
\mathrm{D}\pi\mleft( \mathrm{D}r_{\sigma}\mleft(Y^h\mright)
	- r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^! \Delta \sigma \mright) \mleft(Y^h\mright)}}\mright) \mright)
=
\mathrm{D}{\underbrace{\mleft( \pi \circ r_\sigma \mright)}_{= \pi}}\mleft(Y^h\mright)
=
\mathrm{D}\pi\mleft(Y^h\mright)
\eas
...
\\&\hspace{1.5cm}
	+ r_{\sigma}^*\Biggl(
		...
		- \mleft( \pi^*\nabla^{\mathrm{YM}} \mright)_{\mathcal{r}_{\sigma*}\mleft(Y^h\mright)}\biggl( \mleft(\pi^! \Delta \sigma \mright) \mleft(X^h\mright)\biggr)
\\&\hspace{3cm}
