...
\bas
f^*\mathcal{G} &\to N,\\
(p, g) &\mapsto p \cdot g = p.
\eas
That this action satisfies the properties of an action for all $f$ is trivial, hence we call it the \textbf{trivial action}.
\end{examples}

\begin{examples}{Inner group bundle acting on associated fibre bundles, \newline\cite[\S 1.6, adjustet version of Ex.\ 1.6.4, page 35]{mackenzieGeneralTheory}}{AssocLGACtingOnAssocVec}
Let $P \to M$ be a principal bundle with structural Lie group $G$ over a smooth manifold $M$, and recall Def.\ \ref{def:AssociatedLGB}. Especially, let $H$ be another Lie group, equipped with a $G$-representation $\psi$ on $H$, and $F$ be another smooth manifold, equipped with a smooth left $H$-action $\Psi: H \times F \to F$, such that we have two associated bundles over $M$: $\mathcal{H} \coloneqq P \times_\psi H \stackrel{\pi_{\mathcal{H}}}{\to} M$ and $\mathcal{F} \coloneqq P \times_\Psi F \stackrel{\pi_{\mathcal{F}}}{\to} M$, an associated LGB and fibre bundle, respectively.

Then we have a right $\mathcal{H}$-action on $\mathcal{F}$ given by
\bas
\pi_{\mathcal{F}}^*\mathcal{H} &\to \mathcal{F},\\
\bigl( \mleft[ p, v \mright], \mleft[ p, h \mright] \bigr)
&\mapsto
\mleft[ p, \Psi (h^{-1}, v) \mright]
\eas
for all $p \in P_x$ ($x \in M$), $h \in H$ and $v \in F$.
\end{examples}

\begin{proof}
\leavevmode\newline
We first check again that the action is well-defined.
Thence, let $x \in M$, $p \in P_x$ and $p^\prime \coloneqq p \cdot g^\prime$ be another element of $P_x$, where $g^\prime \in G$. Also let $[p_1,v] \in \mathcal{F}_x$ and $[p_2, h] \in \mathcal{H}_x$; then we have unique elements $q_i, q_i^\prime$ of $G$ such that ($i \in \{1,2\}$)
\bas
p_i &= p \cdot q_i,&
p_i &= p^\prime \cdot q_i^\prime,
\eas
...
[p_1, v] \cdot [p_2, h]
&=
[p, \Psi(q_1, v)] \cdot [p, \psi_{q_2}(h)]
=
\mleft[p, \Psi\mleft( \bigl( \psi_{q_2}(h) \bigr)^{-1}, \Psi(q_1, v) \mright) \mright]
=
\mleft[p, \Psi\mleft( \psi_{q_2}\mleft( h^{-1} \mright), v \mright) \mright].
\eas
On the other hand, using $p^\prime$ as a fixed element, we derive
\bas
[p_1, v] \cdot [p_2, h]
&=
\eas
