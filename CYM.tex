...
\subsection{Exponential map of LGBs}
...
The exponential map is smooth because it describes the flow of left-invariant vector fields as it also happens for Lie groups; see for example \cite[Prop.\ 1.7.12, page 58]{Hamilton} for the Lie group statement. Also recall Cor.\ \ref{cor:LeftInvVfToLAB}, we denote left-invariant vector fields by $X_\nu$ where $\nu \in \Gamma(\mathcal{g})$ due to 1:1 correspondence of $L(\mathcal{G})$ and $\Gamma(\mathcal{g})$.
...
\begin{definitions}{LAB actions, \newline \cite[\S 4.1, simplified version of Def.\ 4.1.1, page 149]{mackenzieGeneralTheory}}{LABACtions}
Let $M, N$ be smooth manifolds, $\mathcal{g} \stackrel{\pi}{\to} M$ an LAB, and $f: N \to M$ a smooth map. Then a \textbf{$\mathcal{g}$-action on $N$} is a vector bundle morphism 
\bas
...
\bas
\Gamma(\mathcal{g}) &\to \Gamma(f^*\mathcal{g}) \to \mathrm{T}N,\\
...
We will later see the canonical example given by fundamental vector fields. Using this we can define LAB representations.
...
\textbf{Maybe introduce as LAB action}
...
As usual for gauge theory we will understand connections as horizontal distributions with a certain symmetry along the fibres to assure that connection forms have a relationship to a form defined on the spacetime/basis itself.\footnote{Recall that connection 1-forms on principal bundles (in the classical gauge theory) can be viewed as a 1-form on the spacetime with values in the Atiyah bundle. We usually also want that the connection acting on a section $X$, denoted by $\nabla X$ in the case of vector bundles, still has values in that vector bundle as a bundle over the spacetime/basis and not over its tangent bundle, but this may be the case without further symmetry of the horizontal distribution.} In order to do so this symmetry has to be similar to the symmetry carried in the vertical structure; recall Def.\ \ref{def:FundVecs} and its remark \ref{rem:FundVecsNotations}. The following is a straightforward generalization of what one knows in the typical formulation of gauge theory, see \textit{e.g.}\ \cite[\S 5.1, part 2 to 4 of Prop.\ 5.1.3, page 258f]{Hamilton}.
...
&\coloneqq
\mleft(\mu_\mathcal{G} \circ \pi^{\mathrm{vert}}\mright)_g(Y)
=
...
\cong
\mathrm{pr}_1^*\mathrm{T}M \oplus \pi^*\mathcal{g}
=
\mathrm{H}\mathcal{G} \oplus \mathrm{V}\mathcal{G},
\eas
where $\mathrm{pr}_2: M \times G \to G$ is the projection onto the second component,
and $\pi^{\mathrm{vert}}$ is then equivalent to $\mathrm{Dpr}_2$. So, we get
\bas
\mu_{\mathcal{G}}^{\mathrm{tot}}
&=
\mathrm{pr}_2^!\mu_G,
\eas
where $\mu_G$ is the Maurer-Cartan form of $G$, and where we made use of that for $(x, g) \in M \times G$ we have
\bas
L_{(x, g)}\bigl( (x, q) \bigr)
&=
\bigl(x, L_g(q)\bigr)
\eas
for all $(x, q) \in M \times G$.
\end{remarks}
...
\eas
Thus, as in Remark \ref{rem:MCFormAGeneralizationOfDerivative} one may say that $\Delta \sigma$ is \textit{the} generalization of vector bundle connections to general LGBs $\mathcal{G}$.
\end{remarks}

\begin{remarks}{Canonical flat Darboux derivative}{DarbouxOnCanonFlat}
Let $G$ be a Lie group with Lie algebra $\mathfrak{g}$, and $\mathcal{G} \coloneqq M \times G \stackrel{\pi}{\to} M$ be the trivial LGB equipped with its canonical flat connection $\mathrm{H}\mathcal{G} \coloneqq \pi^*\mathrm{T}M$. Its LAB $\mathcal{g}$ is also trivial $M \times \mathfrak{g}$, and by using Cor.\ \ref{cor:TLGBAsLGB} we get
\ba\label{trivialLGBSplittingWithCanonFLat}
\mathrm{T}\mathcal{G}
&\cong
\pi^*\mathrm{T}M \oplus \mathfrak{g}
\cong
\pi^*\mathrm{T}M \oplus \pi^*\mathcal{g}
=
\mathrm{H}\mathcal{G} \oplus \mathrm{V}\mathcal{G},
\ea
and for a constant section $\sigma \in \Gamma(\mathcal{G})$, \textit{i.e.}\ $\sigma_x = (x, g)$ for all $x \in M$ ($g \in G$), we have
\bas
\mathrm{D}_x\sigma(X)
&=
(X, 0)
\eas
for all $x \in M$ and $X \in \mathrm{T}_xM$. $\pi^{\mathrm{vert}}$ is obviously the projection onto the second component in Eq.\ \eqref{trivialLGBSplittingWithCanonFLat}, so that we get $\sigma^*\pi^{\mathrm{vert}} \circ \mathrm{D}\sigma = 0$ as expected. Therefore in total
\bas
\Delta \sigma
&=
\mleft(\sigma^* \mu_{\mathcal{G}}^{\mathrm{tot}} \mright) \circ \mathrm{D}\sigma
...
Recall Thm.\ \ref{thm:DiffOfLGBAction}; keeping the same notation as in this theorem, we can now write
\ba
\mathrm{D}_{(p, g)}\Phi(X, Y)
&=
\mathrm{D}_pr_\sigma(X)
	+ \mleft.{\oversortoftilde{\mleft( \mu_{\mathcal{G}}\mright)_{\sigma_x} \bigl(Y - \mathrm{D}_{x}\sigma (\omega)\bigr)}}\mright|_{p \cdot g}.
\ea
\end{remarks}

In fact, the Darboux derivative naturally induces a connection on $\mathcal{g}$ as LAB of $\mathcal{G}$. One may have expected that since $\mathrm{H}\mathcal{G}$ should infinitesimally induce a horizontal distribution on $\mathcal{g}$.
