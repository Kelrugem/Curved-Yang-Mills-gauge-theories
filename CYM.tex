...
\begin{center}
	...
	\end{tikzcd}
\end{center}
as a natural subbundle of $\mathrm{T}\mathcal{P}$ via $\{0\} \times \mathbb{R} \subset \mathbb{R}^2$. In this case we have $\mathcal{P} = \mathcal{G}$, and thus we have $\mathrm{V}\mathcal{P}\cong \mathrm{pr}_1^*\mathcal{g}$ by Cor.\ \ref{cor:TLGBAsLGB}. Hence, we can think of the LAB $\mathcal{g}$ as the vertical bundle.
...
\eas
Let us now investigate the behaviour of this horizontal distribution under push-forwards of right-multiplications; as we already discussed before, we are interested into such push-forwards using sections of $\mathcal{G}$. That is, let $X \in \mathrm{H}_{(x, g)}\mathcal{P}$ and $\sigma$ a (local) section of $\mathcal{G}$.
