...
%&=
%\underbrace{\phi_i \circ \pi_2 \circ \mleft( f^*\phi_i \mright)^{-1}}_{= (f, \mathbb{1}_G)} \circ f^*\phi_i \circ \psi_i^{-1}
%=
%(f, \mathbb{1}_G) \circ
%f^*\phi_i \circ \psi_i^{-1},
%\eas
%$(f, \mathbb{1}_G)$ is clearly a Lie group isomorphism $f^{-1}(U_i) \times G \cong U_i \times G$ of trivial LGBs, and thus the condition about $\pi_2|_x$ being a Lie group isomorphism enforces that we can write
%\bas
...
%e^*( \pi^*f ~ X)
%=
%(f\circ \underbrace{\pi \circ e}_{= \mathbb{1}_M}) ~ e^*X
%=
%f~ e^*X
%\eas
...
%\mleft.e^*X_\nu\mright|_x
%=
%\underbrace{\mathrm{D}_{e_x}L_{e_x}}_{= \mathbb{1}_{\mathrm{V}_{e_x}\mathcal{G}}}(\nu_x)
%=
%\nu_x
%\eas
...
%\begin{center}
	%\begin{tikzcd}[column sep=huge]
		%\mathcal{h} \arrow{d} \arrow{r}{\mathrm{D}_{\mleft( \mathbb{1}_N, e_f \mright)} \pi_2} & \mathcal{g} \arrow{d} \\
		%N \arrow{r}{f} & M
	%\end{tikzcd}
%\end{center}
%which describes fibre-wise a Lie algebra isomorphism, where 
%\bas
%\mathrm{D}_{\mleft( \mathbb{1}_N, e_f \mright)} \pi_2
%&\coloneqq
%\mathrm{D}\pi_2 \circ \mleft( \mathbb{1}_N, e_f \mright)
%=
%\mleft[
...
%\ea
%where $\tau$ is any (local) section\footnote{That is, $f \circ \tau = \mathbb{1}_M$ (locally).} of $f$ with $\tau_x = p$, and $\Phi_\tau$ is the orbit map through $\tau$.
\end{propositions}
...
%\mathrm{D}_g\pi\bigl( Y - \mathrm{D}_x\sigma(\omega) \bigr)
%&=
%\mathrm{D}_g\pi( Y ) - \mathrm{D}_x\underbrace{(\pi \circ \sigma)}_{= \mathbb{1}_M}(\omega)
%=
%\mathrm{D}_g \pi (Y) - \omega
%=
%0.
...
%&=
%\mathrm{D}_{(p, e_x)} \underbrace{\bigl( \Phi \circ \mleft( \mathbb{1}_N, R_\sigma \mright) \bigr)}_{\mathclap{ N * \mathcal{G} \ni (p, g) \mapsto p \cdot g \sigma_x = r_\sigma(p \cdot g) }}
	%\bigl( X, \mathrm{D}_x e(\omega) \bigr)
%\\
%&=
%\mathrm{D}_{(p, e_x)}( r_\sigma \circ \Phi ) \bigl( X, \mathrm{D}_x e(\omega) \bigr)
...
%&=
%\mathrm{D}_p r_\sigma \Bigl(
	%\mathrm{D}_{(p, p)} \bigl(\Phi \circ (\mathbb{1}_N, e\circ f)\bigr)( X, X )
%\Bigr),
%\eas
%but on the diagonal $\mathrm{diag}(N\times N)$ of $N\times N$ we have
%\bas
...
%\mathrm{diag}(N \times N) \ni (p,p)
%\mapsto
%\bigl(\Phi \circ (\mathbb{1}_N, e\circ f)\bigr)(p,p)
%=
%\Phi\mleft(p, e_{f(p)}\mright)
%=
%p \cdot e_{f(p)}
...
$\mathcal{G}$ itself is a principal $\mathcal{G}$-bundle, equipped with its canonical right-action inherited by its multiplication $\mathcal{G}*\mathcal{G} \to \mathcal{G}$. The principal bundle atlas then just consists of the identity map $\mathbb{1}_{\mathcal{G}}$.
...
$\bullet$ Finally, observe that we have locally an isomorphism of every principal bundle $\mathcal{P}$ to $\mathcal{G}$: Fix a principal bundle chart $\varphi: \mathcal{P}|_U \to \mathcal{G}|_U$ ($U$ some open subset of $M$). Then take $H \coloneqq \varphi$, $F \coloneqq \mathbb{1}_{\mathcal{G}|_U}$ and $f \coloneqq \mathbb{1}_U$; by the definition of a principal bundle chart this gives a principal bundle isomorphism. Therefore one could say that every principal bundle is locally "trivial" in the sense of being an LGB; recall Ex.\ \ref{ex:TrivialPrincAsLGB}. 

Additionally using Remark \ref{rem:LGBPrincDefDiscussion} we have locally an isomorphism of $\mathcal{P}$ to a trivial LGB; keeping the same notation as in Remark \ref{rem:LGBPrincDefDiscussion}, choose $H \coloneqq \xi_i$, $F \coloneqq \phi_i$ and $f \coloneqq \mathbb{1}_{U_i}$. Hence, locally every principal bundle is also classical in the sense of Ex.\ \ref{ex:TheCLassicalPrincAsEx}.
\end{remark}
...
g &\mapsto s_{\pi_{\mathcal{G}}(g)} \cdot g,
\eas
is a base-preserving principal bundle isomorphism w.r.t.\ $\mathbb{1}_{\mathcal{G}|_U}$.
\end{lemmata}
...
\leavevmode\newline
The proof is similar to the "classical" statement as \textit{e.g.}\ given in \cite[\S 4.2, proof of Lemma 4.2.7, page 210ff.]{Hamilton}, but generalized since we have to treat a possible non-triviality of $\mathcal{G}|_U$. As already discussed, the orbit map $\Phi_s$ is well-defined because of $\pi_{\mathcal{P}} \circ s = \mathbb{1}_{U}$ such that $\mleft(s_{\pi_{\mathcal{G}}(g)}, g\mright) \in \mathcal{P}*\mathcal{G}$. Via Rem.\ \ref{rem:SmoothnessOfACtionTranslations} we also know that $\Phi_s$ is also smooth,
...
%\pi_{\mathcal{G}}(g),
%\eas
%thus, $\Phi_s$ is base-preserving, similarly it follows that $\Phi_s$ is $\mathcal{G}$-equivariant w.r.t.\ $\mathbb{1}_{\mathcal{G}}$ by making use of $\pi_{\mathcal{G}}(g q) = \pi_{\mathcal{G}}(g)$ for all $g, q \in \mathcal{G}*\mathcal{G}$. Furthermore, $\Phi_s$ is bijective since the right $\mathcal{G}$-action on $\mathcal{P}$ is simply transitive, that is, when we restrict $\Phi_s$ to an $x \in U$, then it is a map
%\bas
%\mathcal{G}_x &\to \mathcal{P}_x,\\
%g &\mapsto s_x \cdot g,
%\eas
%thence, as expected 
and it is point-wise the orbit map $\Phi_{s_x}$ through $s_x$ which is a $\mathcal{G}_x$-equivariant diffeomorphism by Remark \ref{rem:LGBPrincDefDiscussion}. So, $\Phi_s$ is fibre-wise bijective and therefore bijective as a whole since it is base-preserving, and it is $\mathcal{G}$-equivariant w.r.t.\ $\mathbb{1}_{\mathcal{G}}$.
...
&=
\mathbb{1}_{\mathrm{T}M|_U}.
\ea
Usually, we just make use of that without further mention; we repeat this trivial fact in order to emphasize that $\mathrm{D}\sigma$ is injective because $\mathbb{1}_{\mathrm{T}M|_U}$ is bijective, and to emphasize that we have
\bas
...
=
\Bigl( \Phi \circ \mleft( \mathbb{1}_{\mathcal{P}}, \mathrm{pr}_2\circ\sigma^H \mright) \Bigr)(p)
\eas
...
\leavevmode\newline
$A$ has values in $\pi^*\mathcal{g}$, and thus $A_s$ has values in $s^*\pi^*\mathcal{g} \cong (\pi \circ s)^*\mathcal{g} = \mathbb{1}_U^*\mathcal{g} \cong \mleft.\mathcal{g}\mright|_U$.
\end{remark}
...
Due to the fact that $s^*\pi^*\mathcal{G} \cong (\pi \circ s)^*\mathcal{G} = \mathbb{1}_U^*\mathcal{G} \cong \mleft.\mathcal{G}\mright|_U$, and due to what we discussed in Subsection \ref{BasicNotations} about pullback sections, we could rewrite the first map to
\bas
C^\infty \mleft( \mleft.\mathcal{P}\mright|_U; \mleft.\mathcal{G}\mright|_U \mright)^{\mleft.\mathcal{G}\mright|_U} 
&\to 
\Gamma\mleft(\mleft.\mathcal{G}\mright|_U\mright),
...
%\mathcal{r}_{\tau_x*}\bigl( \mathrm{D}_x\sigma(X) \bigr)
	...
	%}}\mright|_{\sigma_x \cdot \tau_x}
%\eas
%for all $x \in M$, $X \in \mathrm{T}_xM$ and $\sigma, \tau \in \Gamma(\mathcal{G})$, where we used that $\pi_{\mathcal{G}} \circ \sigma = \mathbb{1}_M$. Viewing $\mathcal{G}$ as a principal $\mathcal{G}$-bundle we can apply Cor.\ \ref{cor:ModifiedRightPushyCommutesWithProj}. Together with recalling Def.\ \ref{def:FundVecs} we derive
%\bas
%\Delta(\sigma\tau)_x(X)
%&=
%\mleft(\mutot\mright)_{\sigma_x\tau_x}\bigl( \mathrm{D}_x(\sigma \tau)(X) \bigr)
...
&=
{\underbrace{\bigl( \pi_{\mathcal{G}} \circ \gamma(t) \bigr)^!}_{\equiv \mathbb{1}_M^!}}\zeta
=
\zeta
\eas
and under the identification $\gamma^*\pi_{\mathcal{G}}^*\mathcal{g} \cong (\pi_{\mathcal{G}} \circ \gamma)^*\mathcal{g} = \mathbb{1}_M^*\mathcal{g} \cong \mathcal{g}$ we get similarly
\bas
...
Bijectivity follows by $S\circ S = \mathbb{1}_{\mathrm{TT}M}$.
...
&=
\mathrm{D}\underbrace{\mleft( \pi_{\mathrm{T}M} \circ X \mright)}_{= \mathbb{1}_M}
=
\mathbb{1}_{\mathrm{T}M},
\eas
