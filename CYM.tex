...
\begin{center}
	\begin{tikzcd}
		f^*\mathcal{G} \arrow{d} \arrow{r}{\pi_2}& \mathcal{G} \arrow{d}{\pi} \\
		N \arrow{r}{f}& M
	\end{tikzcd}
\end{center}
where $\pi_2: N \times \mathcal{G} \to \mathcal{G}$ is the projection onto the second factor. Actually, $f^*\mathcal{G}$ carries a natural structure as an LGB, as also shown in \cite{PullbackLGBLAB}.
