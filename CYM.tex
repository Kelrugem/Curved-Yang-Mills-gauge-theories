...
\eas
for all $(p, \nu_x) \in \mleft(f^*\mathcal{g}\mright)_p$, where $x \coloneqq f(p)$ and so $\nu_x \in \mathcal{g}_x$, and where we made use of that $\mleft(f^*\mathcal{G}\mright)_p \cong \mathcal{G}_x$ as Lie groups via the projection onto the second component; recall Cor.\ \ref{cor:PullbackLGB}.
\end{remarks}
