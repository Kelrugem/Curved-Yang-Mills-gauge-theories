...
%\mright)
\eas
Let us denote $X_h \coloneqq \pi_h(X)$ and $Y_h \coloneqq \pi_h(Y)$, and $X$ and $Y$ will now be elements of $\mathfrak{X}(\mathcal{P})$, and we rewrite
\bas
\mathcal{r}_{\sigma*}(X)
&=
\mathrm{D}r_{\sigma}(X)
	- r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^* \Delta \sigma \mright) \bigl( \mathrm{D}\pi(X) \bigr) }}\mright).
\eas
...
\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} \mleft( r_{\sigma}^!A \mright),
\eas
whose first equality is a very straight-forward generalization of the commutation of pullbacks and de-Rham differentials; this is easy to check, but in case the reader is interested in an explicit proof, then see either \cite[Appendix, Prop.\ A.1, Eq.\ (A.1)]{My1stpaper} or \cite[Appendix, Prop.\ A.1.1, Eq.\ (A.2)]{MyThesis}. Then \textbf{Nutze hier aus, dass die Vektorfelder horizontal sind, dann fallen die ganzen Nablaterme weg und man hat einfach nur A([]), dann arbeite damit und revertiere das ggf.. Dann auch keine Ableitung vom Ad nötig}
\bas
&\mleft(\mathcal{r}_\sigma^!\mleft(\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) \mright) \mleft(X_h, Y_h\mright)
\\
&=
\mleft(r_\sigma^*\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) 
	\bigl(\mathrm{D}r_\sigma(X_h), \mathrm{D}r_\sigma(Y_h)\bigr)
\\
&\hspace{1cm}
	- \mleft(r_\sigma^*\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) 
	\mleft(\mathrm{D}r_\sigma(X_h), r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^* \Delta \sigma \mright) \bigl( \mathrm{D}\pi(Y_h) \bigr) }}\mright) \mright)
\\
&\hspace{1cm}
	- \mleft(r_\sigma^*\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) 
	\mleft(r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^* \Delta \sigma \mright) \bigl( \mathrm{D}\pi(X_h) \bigr) }}\mright), \mathrm{D}r_\sigma(Y_h) \mright)
\\
&\hspace{1cm}
	+ \mleft(r_\sigma^*\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} A\mright) 
	\mleft(r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^* \Delta \sigma \mright) \bigl( \mathrm{D}\pi(X_h) \bigr) }}\mright), r_{\sigma}^*\mleft({\oversortoftilde{ \mleft(\pi^* \Delta \sigma \mright) \bigl( \mathrm{D}\pi(Y_h) \bigr) }}\mright) \mright).
\eas
\end{proof}
