...
\begin{remarks}{Abstract notation}{AbstractNotationForLeftInvarianceVf}
...
\bas
\mathrm{D}\pi(fX) &= f \mathrm{D}\pi(X) = 0.
\eas
...
Finally, that $L(\mathcal{G}) \to \Gamma\mleft(e^*\mathrm{V}\mathcal{G}\mright)$ is bijective is also clear; we know that $X|_{\mathcal{G}_x}$ ($x \in M$) is a left-invariant vector field on $\mathcal{G}_x$ by Rem.\ \ref{rem:AbstractNotationForLeftInvarianceVf}. Hence, for $g \in \mathcal{G}_x$,
\bas
X|_{\mathcal{G}_x}
&=
...
X_{e^*X},
\eas
and hence injectivity follows; surjectivity simply follows by
\bas
\mleft.e^*X_\nu\mright|_x
&=
...
\nu_x
\eas
for all $\nu \in \Gamma(e^*\mathrm{V}\mathcal{G})$ and $x \in M$.
\end{proof}
...
Let $G \to \mathcal{G} \to M$ be an LGB over a smooth manifold $M$. Then $e^*\mathrm{VF} \to M$ admits the structure as an LAB with structural Lie algebra $\mathfrak{g}$, the Lie algebra of $G$.
\end{corollaries}

\begin{proof}
\leavevmode\newline
As already discussed $e^*\mathrm{VF}$ is a vector bundle. The fibres are given by
\bas
\mleft(e^*\mathrm{VF}\mright)_{x}
&=
...
\mathfrak{g}
\eas
for all $x \in M$, where $e$ is the neutral element of $G$, and we used that $\mathcal{G}_x$ is isomorphic to $G$ as a Lie group. All fibres are Lie algebras of the fibre Lie group, isomorphic to $\mathfrak{g}$. The LAB atlas is quickly constructed: Fix an LGB atlas $\{(U_i, \phi_i \}_{i \in I}$ of $\mathcal{G}$, where $I$ is an (index) set, $(U_i)_{i \in I}$ an open covering of $M$, and $\phi_i: \mathcal{G}|_{U_i} \to U_i \times G$ are LGB isomorphisms. The total derivatives of $\phi_i$ give rise to an LAB atlas for $e^*\mathrm{VF}$, \textit{i.e.}\
\bas
e^!\mleft(\mleft.\mathrm{D}\phi_i\mright|_{\mathrm{VF}}\mright):  &\to \mathrm{T}U_i \times \mathfrak{g},\\
(x, g) &\mapsto \mleft( x, \phi_{i, f(x)}(g) \mright),
\eas
where $\phi_{i, f(x)}: \mathcal{G}_{f(x)} \to G$ are the Lie group isomorphisms as defined in Def.\ \ref{def:LieGroupBundle}.
\end{proof}
