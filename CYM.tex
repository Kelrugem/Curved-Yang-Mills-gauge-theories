...
Let $M, N$ be smooth manifolds, $\mathcal{G} \stackrel{\pi}{\to} M$ an LGB over $M$ and $f: N \to M$ a smooth map. Then $f^*\mathcal{G}$ has a unique LGB structure such that the projection $\pi_2: N \times \mathcal{G} \to \mathcal{G}$ onto the second factor is an LGB isomorphism.
\end{corollaries}

\begin{remark}
\leavevmode\newline
The mentioned reference, \cite[\S 2.3, discussion around Prop.\ 2.3.1, page 63ff.]{mackenzieGeneralTheory}, is rather general and for Lie groupoids. If the reader is only interested into LGBs, then see \cite[\S 3, Thm.\ 3.1]{PullbackLGBLAB}.
\end{remark}
