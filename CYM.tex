...
Following Remark \ref{rem:PullbackTotMCForm} and its notation and results, we can calculate $\Delta^{f^*\mathcal{G}}\mleft( f^*\sigma \mright) \in \Omega^1(N; f^*\mathcal{g})$ for $\sigma \in \mathcal{G}$, first we write
\bas
...
f^*\Delta^G,
\eas
where $\Delta^G$ on the right hand side is the "classical" Darboux derivative as in Remark \ref{rem:MCFormAGeneralizationOfDerivative} related to $\mu_G$. This emphasizes why we can speak of a canonical flat derivative due to the fact that $\mu_G$ is flat (Maurer-Cartan equation).
