...
\nabla^{\mathcal{G}}
	+ \mathrm{ad}_{\Delta \sigma}.
\eas
The remaining part of the statement are trivial and straight-forward calculations, making use of Def.\ \ref{def:YangMillsConnection}.
\end{proof}
...
Via Thm.\ \ref{thm:TotMaurerIsMultiplicativeIfSEqual0} and Lemma \ref{lem:YangMillsConnAnEhresmann} we therefore concluded that compatibility condition \eqref{CondSGleichNullLAB} holds if and only if the total Maurer-Cartan form is multiplicative. Let us therefore now turn to compatibility condition \eqref{CondKruemmungmitBLAB}.
...
\eas
for all $g \in \mathcal{G}$. We then say that $\mu_{\mathcal{G}}^{\mathrm{tot}}$ solves the \textbf{generalized Maurer-Cartan equation (w.r.t.\ $\zeta$)}.
\end{theorems}
...
On degree 0 it is a map $\Omega^\bullet(M; \mathcal{g}) \to \Omega^\bullet\mleft(\mathcal{G}; \pi^{*}_{\mathcal{g}}\mright)$ given for $\xi \in \Omega^\bullet(M; \mathcal{g})$ by
\bas
...
\leavevmode\newline
The idea of the proof is similar to the proof of a statement about when two multiplicative 2-forms are equal, see \textit{e.g.}\ \cite[\S 3, Cor.\ 3.4]{bursztyn2004integration}. 

$\bullet$ That is, we now first discuss when 
\bas
...
For a given $g \in \mathcal{G}_x$ ($x \in M$) define
\bas
\mathbb{R} &\to \mathcal{G}_x,\\
t &\mapsto \gamma(t) \coloneqq \sigma \cdot \e^{t\nu}
\eas

 take any curve $\gamma: I \to \mathcal{G}_x$ () so that $\gamma(0) = g$.
\end{proof}
