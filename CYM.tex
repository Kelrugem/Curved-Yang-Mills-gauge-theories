...
Of course, we expect certain properties, naturally generalized by our previous discussion; however, in order to prove these we need to discuss the Darboux derivative w.r.t.\ Yang-Mills connections.
%\footnote{Many of the following properties can actually be showed w.r.t.\ every Ehresmann connection, not just the Yang-Mills ones.} 
Let us start with an auxiliary result and then a Leibniz rule; especially recall all the discussion around the modified push-forward $\mathcal{r}$ via right-translations, Def.\ \ref{def:WeModTheRIghtPush}.
...
\mathrm{d}^{\pi^*\nabla^{\mathrm{YM}}} \mleft( r_{\sigma}^!A \mright),
\eas
whose first equality is a very straight-forward generalization of the commutation of pullbacks and de-Rham differentials; this is easy to check, but in case the reader is interested in a proof, then see either \cite[Appendix, Prop.\ A.1, Eq.\ (A.1)]{My1stpaper} or \cite[Appendix, Prop.\ A.1.1, Eq.\ (A.2)]{MyThesis}.
\end{proof}
