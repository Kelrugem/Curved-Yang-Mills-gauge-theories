...
\begin{proof}
\leavevmode\newline
This result can be proven similarly as for Lie group based principal bundles; however smoothness needs to be discussed a bit. 

$\bullet$ First of all, due to the fact that $H$ is base-preserving we know that $H(p)$ is in the same fibre as $p$ ($p \in \mathcal{P}$), and due to that the $\mathcal{G}$-action on $\mathcal{P}$ is simply transitive there is a unique element of $\mathcal{G}$, denoted by $\sigma^H_p$, such that
\bas
H(p)
&=
p \cdot \sigma_p^H
...
Let us first show smoothness of $p \mapsto \sigma_p^H$; recall first Remark \ref{rem:LGBPrincDefDiscussion}: We can write
\bas
H(p) &= \Phi_p \mleft( \sigma_p^H \mright),
\eas
where $\Phi_p: \mathcal{G}_x \to \mathcal{P}_x$ is the orbit through $p \in \mathcal{P}_x$ ($x \in M$). By Remark \ref{rem:LGBPrincDefDiscussion}, $\Phi_p$ is a $\mathcal{G}_x$-equivariant diffeomorphism, especially invertible, so that
\bas
...
\eas
which is a map over $\pi$ by construction. If we can show smoothness of $\Phi^{-1}$, then smoothness of $\sigma^H$ follows due to smoothness of $H$ and
\bas
\sigma_p^H
&=
...
$\bullet$ By definitions, if we fix a point $x \in M$, then $H_x$ is an element of $\sAut(\mathcal{P}_x)$, where $\mathcal{P}_x$ is viewed as principal $\mathcal{G}_x$-bundle. Elements of $C^\infty(\mathcal{P}; \mathcal{G})^{\mathcal{G}}$ restricted to $\mathcal{P}_x$ are precisely the elements of $C^\infty(\mathcal{P}_x; \mathcal{G}_x)^{\mathcal{G}_x}$
\end{proof}
