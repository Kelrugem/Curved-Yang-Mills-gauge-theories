...
\begin{equation}\label{LiegroupStructureOnFibresofAssociated}
...
\leavevmode\newline
The proof is similar to the "classical" statement as \textit{e.g.}\ given in \cite[\S 4.2, proof of Lemma 4.2.7, page 210ff.]{Hamilton}, but generalized since we have to treat a possible non-triviality of $\mathcal{G}|_U$. As already discussed, the orbit map $\Phi_s$ is well-defined because of $\pi_{\mathcal{P}} \circ s = \mathbb{1}_{U}$ such that $\mleft(s_{\pi_{\mathcal{G}}(g)}, g\mright) \in \mathcal{P}*\mathcal{G}$. Via Rem.\ \ref{SmoothnessOfACtionTranslations} we also know that $\Phi_s$ is also smooth,
