...
\eas
for all $x \in M$, where we also used Eq.\ \eqref{LABActionAlongFibres}; that is, we expect that an LAB action is equivalent to a fibre-wise Lie algebra action. Due to a possible lack of manifold structure on $f^{-1}(\{x\})$ we did not restrict to $x\in M$ to avoid technical difficulties. However, $f$ will be the projection of a bundle later, and in that case the fibre $f^{-1}(\{x\})$ is an embedded submanifold of $N$. As argued in Remark \ref{rem:LocalLGBAction}, the $\mathcal{G}$-action is a $\mathcal{G}_x$-action restricted on $f^{-1}(\{x\})$. By what we know about Lie group actions we immediately know that we have the aforementioned Lie algebra homomorphism, and so also Eq.\ \eqref{ActionLieAlgebroidButNonTrivial}, in total a fibre-wise Lie algebra action.
\end{remarks}
...
%Making use of $\mathrm{V}_vV \cong V_x$ for all $v\in V_x$ ($x \in M$), it is clear that $p^*V \cong \mathrm{V}V$ as vector bundles.
As argued in Rem.\ \ref{rem:LABHomomorpInActionVariants}, since $\mathcal{g}$ acts via a representation on a bundle $V$ now, $\widetilde{\psi}: p^*\mathcal{g} \to \mathrm{V}V$ will be an LAB action 
\end{proof}
